...
function dataUrlBytes(dataUrl: string) {
  const match = /^data:(image\/(?:png|jpeg|jpg));base64,([a-z0-9+/=]+)$/i.exec(
    dataUrl,
  );
  return match
...
async function makePdf({
  ...
  const navy = rgb(0.204, 0.451, 0.886),
    paleBlue = rgb(0.839, 0.89, 0.976),
    slate = rgb(0.4, 0.43, 0.48),
  ...
  const centeredText = (
    ...
    text(
      value,
      x + (width - font.widthOfTextAtSize(safeText(value), size)) / 2,
      top,
      size,
      isBold,
      color,
  ...
  try {
    const logo = await pdf.embedPng(
      await readFile(
        path.join(process.cwd(), "public", "logo-with-company-name.png"),
      ),
    );
    const size = fit(logo, 185, 70);
    page.drawImage(logo, { x: 20, y: H - 26 - size.height, ...size });
  } catch {
    text("NanyangGifts Pte. Ltd.", 24, 48, 10, true);
  }
  [
    ...
    "Singapore 536982",
    "+65 6282 1225",
    "sales@nanyanggifts.com",
    "www.nanyanggifts.com",
  ...
  [
    client.company || client.name || "Customer",
  ...
  for (const row of rows) {
    ...
    const artworkHeight = artworkSize
      ? artworkTopOffset + artworkSize.height + 8
      : 0;
    ...
    if (image && artworkSize) {
      page.drawImage(image, {
        x: cols[0] + 4 + (descriptionWidth - artworkSize.width) / 2,
        y: H - top - artworkTopOffset - artworkSize.height,
        ...artworkSize,
      });
    }
    names.forEach((item, index) =>
      text(item, 22, top + 15 + index * 11, 8.5, index === 0),
    );
    details.forEach((item, index) =>
      text(item, cols[1] + 5, top + 15 + index * 10, 7.5),
    );
    centeredText(
      row.qty.toLocaleString("en-SG"),
      cols[2],
      top + 15,
      cols[3] - cols[2],
      8.5,
  ...
  text("Payment Details:", 18, summaryTop + 14, 9);
  text("Payee - NANYANGGIFTS PTE. LTD.", 18, summaryTop + 28, 9);
  text("Bank - OCBC Bank 551-872-096-001", 18, summaryTop + 42, 9);
  text("PayNow UEN: 201426646R", 18, summaryTop + 56, 9);
  text("SUBTOTAL", 350, summaryTop + 14, 9, false, navy);
  text(
    money(subtotal),
    565 - regular.widthOfTextAtSize(money(subtotal), 9),
    summaryTop + 14,
    9,
  );
  ...
  text(
    money(gst),
    565 - regular.widthOfTextAtSize(money(gst), 9),
    summaryTop + 28,
    9,
  );
  ...
  text("THANK YOU.", 500, summaryTop + 82, 9, true, navy);
  ...
  ["RATE", "GST", "NET"].forEach((label, index) =>
    centeredText(
      label,
      gstColumns[index],
      gstTop + 25,
      gstColumns[index + 1] - gstColumns[index],
      8,
      true,
    ),
  );
  line(18, gstTop + 44, 577, rgb(0.78, 0.78, 0.78), 0.3);
  ["GST @ 9%", money(gst), money(subtotal)].forEach((value, index) =>
    centeredText(
      value,
      gstColumns[index],
      gstTop + 55,
  ...
  pages.forEach((outputPage, index) => {
    ...
    outputPage.drawText(pageNumber, {
      x: W - 38 - regular.widthOfTextAtSize(pageNumber, 8),
      y: 14,
      size: 8,
      font: regular,
      color: slate,
...
export async function POST(req: NextRequest) {
  ...
  if (!subitems.length)
    return NextResponse.json(
      {
        error:
          "No eligible subitems with Quoted, Shortlisted, or Awarded status",
      },
      { status: 400 },
    );
  const artworkById = new Map(
    artworks
      .filter(
        (item): item is ArtworkInput =>
          typeof item?.subitemId === "string" &&
  ...
  const { data: profile } = await supabase
    ...
    .eq("id", user.id)
    .maybeSingle();
  const rows = subitems.map((item: any) => {
    const qty = numberValue(item.qty) || 1;
    const unitPrice =
      numberValue(item.up) || (qty > 0 ? numberValue(item.price) / qty : 0);
    return {
      ...
      artwork: artworkById.get(item.id),
    };
  });
  const { data: quotationNumber, error: sequenceError } = await supabase.rpc(
    "next_sample_estimate_number",
  );
  if (sequenceError || !Number.isInteger(Number(quotationNumber))) {
    return NextResponse.json(
      {
        error:
          sequenceError?.message || "Could not allocate a quotation number.",
      },
      { status: 500 },
    );
  }
  const number = Number(quotationNumber);
  const filename = `Quotation ${number} - ${
  ...
  const { error: activityError } = await supabase.from("activity_log").insert({
    ...
    meta: {
      ...
      quotationNumber: number,
      storagePath,
      subitemIds: subitems.map((item: any) => item.id),
    },
  ...
  if (activityError)
    return NextResponse.json(
      {
        error: `Quote generated, but the activity log could not be updated: ${activityError.message}`,
      },
      { status: 500 },
    );
  return NextResponse.json({
