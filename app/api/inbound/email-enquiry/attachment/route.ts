...
function existingAttachments(value: unknown): StoredAttachment[] {
  ...
  try {
    ...
    return Array.isArray(parsed)
      ? parsed.filter((item): item is StoredAttachment =>
...
function queueAttachments(value: unknown): StoredAttachment[] {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const externalId = String(
      formData.get("externalId") ?? formData.get("messageId") ?? "",
    ).trim();
    const attachmentId = String(formData.get("attachmentId") ?? "").trim();
    const file = formData.get("file");
    if ((!clientId && !reviewId) || !externalId || !(file instanceof File)) {
      return NextResponse.json(
        {
          error:
            "clientId or reviewId, externalId/messageId, and file are required.",
        },
        { status: 400 },
      );
    }
    ...
    if (file.size > MAX_FILE_SIZE) {
      return NextResponse.json(
        { error: "Email attachments must be 3.5 MB or smaller." },
        { status: 413 },
      );
    }
    ...
    if (reviewId) {
      ...
      if (!review)
        return NextResponse.json(
          { error: "Matching email review record was not found." },
          { status: 404 },
        );
      const payload =
        review.payload &&
        typeof review.payload === "object" &&
        !Array.isArray(review.payload)
          ? (review.payload as Record<string, unknown>)
          : {};
      const attachments = queueAttachments(payload.attachments);
      ...
      const storagePath = `email-review/${reviewId}/${randomUUID()}.${extension}`;
      ...
      const stored: StoredAttachment = {
        ...
        createdAt,
        createdThrough: "Received in Email Review through Make",
        sourceKey,
      };
      const { error: updateError } = await supabaseAdmin
        .from("email_review_queue")
        .update({
          payload: { ...payload, attachments: [...attachments, stored] },
    ...
    if (!ingestion)
      return NextResponse.json(
        { error: "Matching email ingestion was not found." },
        { status: 404 },
      );
    ...
    if (duplicate)
      return NextResponse.json({ ok: true, duplicate: true, file: duplicate });
    ...
    const { error: updateError } = await supabaseAdmin
      ...
      .update({
        custom_fields: {
          ...customFields,
          logoRequirementsFile: JSON.stringify([...attachments, stored]),
        },
      })
      .eq("id", clientId);
    if (updateError) {
