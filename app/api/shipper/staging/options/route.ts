...
export async function GET(request: NextRequest) {
  ...
  const { data: subitems, error } = await supabaseAdmin.from("subitems").select("id, name, client_id").order("name"); if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  const clientIds = [...new Set((subitems ?? []).map((item) => item.client_id).filter(Boolean))];
  const { data: clients } = clientIds.length ? await supabaseAdmin.from("clients").select("id, name, group_id, custom_fields").in("id", clientIds) : { data: [] };
  ...
  const withPermission = (item: { id: string; name: string; client_id: string | null }) => ({ ...item, canPush: assignedSubitemIds.has(item.id) || (!!item.client_id && (assignedClientIds.has(item.client_id) || pmClientIds.has(item.client_id))) });
