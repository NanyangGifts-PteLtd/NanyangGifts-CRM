...
const LABEL_FIELDS: Record<string, LabelField> = {
  ...
  importance: {
    table: "clients",
    valueColumn: "importance",
    optionIdColumn: "importance_option_id",
  },
  progress: {
    table: "clients",
    valueColumn: "progress",
  ...
  payment_status: {
    table: "subitems",
    valueColumn: "payment_status",
    optionIdColumn: "payment_status_option_id",
  },
  mode_of_payment: {
    table: "subitems",
    valueColumn: "mode_of_payment",
  ...
  additional_cost_reason: {
    table: "additional_costs" as "clients",
    valueColumn: "reason",
    optionIdColumn: "reason_option_id",
  },
...
async function findOption(code: string, name: string) {
  const { data: group, error: groupError } = await supabaseAdmin
    ...
    .maybeSingle();
  if (groupError || !group)
    return { error: groupError?.message ?? "Label group was not found." };
  ...
  if (trackingField) {
    ...
    for (const row of data ?? []) {
      ...
      const { error: updateError } = await supabaseAdmin
        .from("clients")
        .update({
          custom_fields: { ...(row.custom_fields ?? {}), [trackingField]: "" },
        })
        .eq("id", row.id);
...
export async function POST(request: NextRequest) {
  ...
  if (
    ...
    ![
      ...
      "subitem_subprogress",
      ...Object.keys(TRACKING_CUSTOM_FIELD_BY_CODE),
    ].includes(code)
  ) {
    return NextResponse.json(
      { error: "Invalid label option." },
      { status: 400 },
    );
  }
