...
function formulaValue(row: ShipperRow, key: string) {
  ...
  if (key === "freight_cost")
    return (
      numberValue(row.chargeable_weight_kg) *
      numberValue(row.freight_unit_price)
    );
  if (key === "total_cost")
    return (
      numberValue(row.freight_cost) +
      numberValue(row.gst) +
      numberValue(row.other_fees)
    );
  return row[key as keyof ShipperRow];
}

function rowWithCellValue(
  row: ShipperRow,
  field: string,
  value: string,
): ShipperRow {
  const next = {
    ...row,
    [field]: field === "cell_fills" ? JSON.parse(value) : value,
  } as ShipperRow;
  if (field === "qty" || field === "up")
    next.value = numberValue(next.qty) * numberValue(next.up);
  if (field === "chargeable_weight_kg" || field === "freight_unit_price")
    next.freight_cost =
      numberValue(next.chargeable_weight_kg) *
      numberValue(next.freight_unit_price);
  if (
    field === "chargeable_weight_kg" ||
    field === "freight_unit_price" ||
    field === "gst" ||
    field === "other_fees"
  )
    next.total_cost =
      numberValue(next.freight_cost) +
      numberValue(next.gst) +
      numberValue(next.other_fees);
  return next;
}
...
function DateCell({
  ...
  if (!editable)
    return (
      <div
        title={tooltip}
        className="flex min-h-[42px] items-center justify-center px-2"
      >
        {formatDmy(value) || "-"}
      </div>
    );
  return (
    <input
      ...
      title={tooltip || "Use DD/MM/YYYY"}
      placeholder="DD/MM/YYYY"
      className={`min-h-[42px] w-full border-0 bg-transparent px-2 text-center outline-none focus:bg-blue-50 ${error ? "bg-red-50 text-red-700" : ""}`}
    />
  );
}
...
function SpreadsheetTextCell({
  ...
  return (
    <textarea
      ...
      className="block min-h-[42px] w-full resize-none overflow-hidden border-0 bg-transparent px-2 py-2 text-center text-[13px] whitespace-pre-wrap break-words outline-none focus:bg-blue-50"
    />
  );
}
...
function parseRemarks(value: unknown) {
  ...
  const images: string[] = [];
  const text = raw
    .replace(IMAGE_MARKER, (_match, url: string) => {
      images.push(url);
      return "";
    })
    .replace(/\n{3,}/g, "\n\n")
...
function RemarksCell({
    ...
    <div key={url} className="relative mt-2 w-fit max-w-full">
      <a href={url} target="_blank" rel="noreferrer" className="block">
        <img
          src={url}
          alt="Attached remark"
          className="max-h-48 max-w-[240px] rounded border border-slate-200 object-contain"
        />
      </a>
  ...
  return (
    <FileDropTarget
      ...
      disabled={uploading}
      className="min-h-[42px] min-w-[240px]"
    >
      <div
        className="min-h-[42px] min-w-[240px]"
        onPaste={(event) => {
...
export default function ShipperGrid({ rows, mode, token }: ShipperGridProps) {
  ...
  const columns = [
    ...
    {
      key: "air_received",
      label: "空运收到 / Air received",
      editableByPm: true,
      editableByShipper: false,
    },
    {
      key: "sea_received",
      label: "海运收到 / Sea received",
      editableByPm: true,
      editableByShipper: false,
    },
  ];
  async function saveCell(row: ShipperRow, field: string, value: string) {
    ...
    setGridRows((previous) =>
      previous.map((item) =>
        item.id === row.id ? rowWithCellValue(item, field, value) : item,
      ),
    );
  ...
  useEffect(() => {
    setCellFills(
      Object.fromEntries(
        rows.flatMap((row) =>
          Object.entries(row.cell_fills ?? {}).map(([column, color]) => [
            `${row.id}:${column}`,
            color,
          ]),
        ),
  ...
  const selectionBounds = selection && {
    ...
    firstCol: Math.min(selection.startCol, selection.endCol),
    lastCol: Math.max(selection.startCol, selection.endCol),
  };
  const cellSelected = (rowIndex: number, colIndex: number) =>
    !!selectionBounds &&
    rowIndex >= selectionBounds.firstRow &&
    rowIndex <= selectionBounds.lastRow &&
    colIndex >= selectionBounds.firstCol &&
    colIndex <= selectionBounds.lastCol;
  const selectCell = (
    ...
    if (event.shiftKey && selection)
      setSelection((previous) =>
        previous
          ? { ...previous, endRow: rowIndex, endCol: colIndex }
          : previous,
      );
    else
      setSelection({
  ...
  const fillSelectedCells = (color: string) => {
    ...
    setCellFills((previous) => {
      ...
      for (
        let rowIndex = selectionBounds.firstRow;
        rowIndex <= selectionBounds.lastRow;
        rowIndex += 1
      ) {
        for (
          let colIndex = selectionBounds.firstCol;
          colIndex <= selectionBounds.lastCol;
          colIndex += 1
        ) {
      ...
      affectedRows.forEach((rowId) => {
        ...
        if (!row) return;
        const rowFills = Object.fromEntries(
          Object.entries(next)
            .filter(([key]) => key.startsWith(`${rowId}:`))
            .map(([key, fill]) => [key.slice(rowId.length + 1), fill]),
        );
        void saveCell(row, "cell_fills", JSON.stringify(rowFills)).catch(
          (error) => console.error("Failed to save cell fill", error),
        );
      });
      return next;
    });
  ...
  return (
    <div className="w-full overflow-auto">
      <div className="sticky left-0 z-40 mb-1 flex h-10 max-w-[calc(100vw-1rem)] items-center gap-1 rounded-md border border-slate-300 bg-white p-1.5 shadow-md">
        {selectionBounds && isFillPaletteOpen ? (
          <>
            <span className="shrink-0 px-1 text-xs font-medium text-slate-500">
              Fill
            </span>
            <div className="flex min-w-0 flex-1 gap-1 overflow-x-auto py-0.5">
              {fillColors.map((color) => (
                <button
                  key={color}
                  type="button"
                  onClick={() => fillSelectedCells(color)}
                  className="h-5 w-5 shrink-0 rounded border border-slate-300 transition hover:scale-110"
          ...
          <button
            ...
            title={
              selectionBounds
                ? "Open fill palette"
                : "Select the top-left cell and choose a fill colour"
            }
          >
      ...
      <div className="rounded-md border border-slate-300 bg-white shadow-sm">
        <table className="min-w-[2400px] border-separate border-spacing-0 text-[13px] text-black">
          <thead>
            <tr>
              <th className="sticky left-0 top-0 z-30 min-w-12 border-b-2 border-r-2 border-slate-400 bg-slate-100 px-2 py-2 text-center text-xs font-semibold text-slate-700 shadow-[2px_2px_4px_rgba(15,23,42,0.12)]">
                #
              </th>
              {columns.map((col) => (
                <th
            ...
            {gridRows.length === 0 ? (
              ...
              gridRows.map((row, index) => (
                <tr key={row.id} className="bg-white text-black align-top">
                  <td className="sticky left-0 z-10 min-w-12 border-b border-r-2 border-slate-300 bg-slate-50 px-2 text-center text-xs text-slate-500 shadow-[2px_0_4px_rgba(15,23,42,0.10)]">
                    {index + 1}
                  </td>
                  {columns.map((col, colIndex) => {
                    ...
                    return (
                      <td
                        key={col.key}
                        onMouseDown={(event) =>
                          selectCell(event, index, colIndex)
                        }
                        onMouseEnter={() => {
                          if (dragSelecting && selection)
                            setSelection((previous) =>
                              previous
                                ? {
                                    ...previous,
                      ...
                      >
                        ...
                        col.key === "shipper_remarks" ? (
                          ...
                          <DateCell
                            value={row.info_provided_date}
                            editable={editable}
                            tooltip={dateCellTooltip(row.info_provided_date)}
                            onSave={(nextValue) =>
                          ...
                          <select
                            value={String(value ?? "")}
                            onChange={(event) =>
                              void saveCell(row, col.key, event.target.value)
                            }
                            className="min-h-[42px] w-full border-0 bg-transparent px-2 text-center outline-none focus:bg-blue-50"
                          >
                            <option value="" />
                          ...
                          <div className="flex min-h-[42px] items-center justify-center px-2">
                            {display(formulaValue(row, col.key))}
                          </div>
                        )}
                      </td>
                    );
