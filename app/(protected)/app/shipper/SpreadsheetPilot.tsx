...
const shipmentInputFields = [...shipmentFields].filter(
  (field) => !formulaFields.has(field),
);
const clearableFields = columns
  .map((column) => String(column.id))
  .filter((field) => field !== "__lock" && !formulaFields.has(field));
...
function formulaValues(values: Record<string, unknown>) {
  ...
  const freight =
    weight !== null && freightRate !== null ? weight * freightRate : null;
  const gst = numberOrNull(values.gst);
  const other = numberOrNull(values.other_fees);
  const total =
    freight !== null && gst !== null && other !== null
      ? freight + gst + other
      : null;
  ...
  return {
    ...
    total_cost: total,
    value: qty !== null && unitPrice !== null ? qty * unitPrice : null,
  };
}
...
function formattedValue(key: string, value: unknown) {
  ...
  if (number !== null && currencyFields.has(key))
    return new Intl.NumberFormat("en-SG", {
      minimumFractionDigits: 2,
      maximumFractionDigits: 2,
    }).format(number);
  if (number !== null && numberFields.has(key))
    return new Intl.NumberFormat("en-SG", { maximumFractionDigits: 2 }).format(
      number,
    );
  return String(value);
}
...
function remarkImages(value: unknown) {
  return [
    ...String(value ?? "").matchAll(/\[\[shipper-image:([^\]]+)\]\]/g),
  ].map((match) => match[1]);
}
...
function attachmentSource(url: string) {
  ...
  return index < 0
    ? url
    : `/api/shipper/image?path=${encodeURIComponent(url.slice(index + marker.length))}`;
}
...
function fillTextColor(fill: string | undefined) {
  ...
  const rgb = [fill.slice(1, 3), fill.slice(3, 5), fill.slice(5, 7)].map(
...
function validationMessage(field: string, value: unknown) {
  ...
  if (
    ...
    (currencyFields.has(field) && !formulaFields.has(field))
  ) {
    return Number.isFinite(Number(value))
      ? null
      : "This cell must contain a number.";
  }
  if (dateFields.has(field))
    return /^\d{4}-\d{2}-\d{2}$/.test(String(value))
      ? null
      : "Dates must use YYYY-MM-DD.";
  if (["channel", "sea_or_air"].includes(field))
    return seaOrAirOptions.includes(String(value))
      ? null
      : "Choose 空运, 海运, or 海运/小包.";
  if (field === "tax_refund")
    return selectOptions.tax_refund.includes(String(value))
      ? null
      : "Choose 退 or X.";
  return null;
}
...
function movementAtTextEdge(
  element: HTMLTextAreaElement,
  key: string,
): EditorMovement | undefined {
...
function DropdownEditor({
  ...
  value: GridCell;
  onFinishedEditing: (next?: GridCell, movement?: EditorMovement) => void;
}) {
  ...
  return (
    <select
      ...
      className="h-full w-full border-0 bg-white px-2 outline-none"
    >
      <option value="" />
      {options.map((option) => (
...
function DateEditor({
  ...
  return (
    <input
      ...
      onKeyDown={(event) => {
        const movement: Record<string, EditorMovement> = {
          ArrowUp: [0, -1],
          ArrowDown: [0, 1],
          ArrowLeft: [-1, 0],
          ArrowRight: [1, 0],
      ...
      className="h-full w-full border-0 bg-white px-2 text-center outline-none"
    />
  );
}
...
function TextEditor({
  ...
  return (
    <textarea
      ...
      className="h-full w-full resize-none border-0 bg-white px-2 py-1 text-center outline-none"
    />
  );
}
  ...
  return (
    <textarea
      ...
      onKeyDown={(event) => {
        ...
        if (next) {
          event.preventDefault();
          finish(event.currentTarget.value, next);
          return;
        }
        if (event.key === "Enter" && (event.ctrlKey || event.metaKey)) {
          event.preventDefault();
...
export function SpreadsheetPilot({
  shipperId,
  mode = "internal",
}: {
  shipperId: string;
  ...
  const [workbookId, setWorkbookId] = useState<string | null>(
    () => cachedSnapshot?.workbookId ?? null,
  );
  ...
  const pendingSaves = useRef(
    new Map<
      string,
      { values: Record<string, unknown>; replaceValues: boolean }
    >(),
  );
  ...
  const load = useCallback(async () => {
    ...
    const response = await fetch(
      `/api/shipper/spreadsheet?shipperId=${shipperId}`,
    );
    const result = await response.json();
    if (!response.ok)
      return setError(result.error ?? "Could not load spreadsheet");
    ...
    if (
      ...
      revisionAtStart !== localEditRevision.current ||
      ...
      channel = supabase
        ...
        .on(
          "postgres_changes",
          {
            event: "*",
            schema: "public",
            table: "shipper_spreadsheet_rows",
  ...
  useEffect(() => {
    const nextDue = rows
      ...
      .sort(
        (left, right) =>
          new Date(left.auto_lock_at!).getTime() -
          new Date(right.auto_lock_at!).getTime(),
      )[0];
    ...
    const timer = window.setTimeout(async () => {
      const response = await fetch("/api/shipper/spreadsheet", {
        ...
        body: JSON.stringify({ shipperId, rowId: nextDue.id, isLocked: true }),
      });
      const result = await response.json();
      if (!response.ok) {
        void load();
  ...
  const toggleRowLock = useCallback(
    async (record: Row) => {
      ...
      const result = await response.json();
      if (!response.ok)
        return setError(result.error ?? "Could not update row lock");
      ...
      setRows((current) =>
        current.map((row) =>
          row.id === record.id
            ? {
                ...row,
                is_locked: result.row.is_locked,
                auto_lock_at: result.row.auto_lock_at,
                version: result.row.version,
              }
            : row,
        ),
  ...
  const flushPendingSaves = useCallback(async () => {
    ...
    try {
      const results = await Promise.all(
        pending.map(async ([rowId, pendingSave]) => {
          const response = await fetch("/api/shipper/spreadsheet", {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              ...
              replaceValues: pendingSave.replaceValues,
            }),
          });
      ...
      if (results.some(({ response }) => !response.ok)) {
        failed = true;
        setError(
          results.find(({ response }) => !response.ok)?.result?.error ??
            "Could not save one or more cells",
        );
      } else {
        const savedRows = new Map<
          string,
          { auto_lock_at: string | null; version: number }
        >(results.map(({ result }) => [result.row.id, result.row]));
        setRows((current) =>
          current.map((row) =>
            savedRows.has(row.id)
              ? {
                  ...row,
                  auto_lock_at: savedRows.get(row.id)!.auto_lock_at,
                  version: savedRows.get(row.id)!.version,
                }
              : row,
          ),
        );
  ...
  const getCellContent = useCallback(
    ([col, row]: Item): GridCell => {
      ...
      if (key === "__lock") {
        const autoLockPending = Boolean(
          record?.auto_lock_at &&
          new Date(record.auto_lock_at).getTime() > Date.now(),
        );
        const symbol = hasRowContent(record)
          ? record!.is_locked
            ? "🔒"
            : autoLockPending
              ? "⏳"
        ...
        return {
          ...
          themeOverride: record?.is_locked
            ? { bgCell: "#e2e8f0", textDark: "#475569" }
            : undefined,
        };
      }
      if (!record) {
      ...
      const isMergedFollower = Boolean(
        record.shipment_group_id &&
        previous?.shipment_group_id === record.shipment_group_id &&
        shipmentFields.has(key),
      );
      const values: Record<string, unknown> = hasRowContent(record)
        ? { ...record.values, ...formulaValues(record.values) }
        : {};
      ...
      const readOnly =
        (record.is_locked && !alwaysEditableAfterLockFields.has(key)) ||
        formulaFields.has(key) ||
        (mode === "shipper" && !shipperEditableFields.has(key));
      return {
        kind: GridCellKind.Text,
        ...
        themeOverride: (() => {
          ...
          if (record.is_locked && !alwaysEditableAfterLockFields.has(key))
            return { bgCell: "#f1f5f9", textDark: "#475569" };
          return formulaFields.has(key)
            ? { bgCell: "#fff7d6", textDark: "#7c4a03" }
            : undefined;
        })(),
        field: key,
      } as GridCell;
  ...
  const rowHeightForIndex = useCallback(
    (rowIndex: number) => {
      ...
      const lines = columns.slice(1).reduce((maximum, column) => {
        ...
        const columnWidth =
          "width" in column && typeof column.width === "number"
            ? column.width
            : 120;
  ...
  const drawMergedShipmentCell = useCallback(
    (
      args: {
          ...
          if (image.complete && image.naturalWidth > 0) {
            ...
            if (images.length > 1) {
              ...
              const badgeWidth = Math.max(
                21,
                ctx.measureText(badgeText).width + 10,
              );
      ...
      const isNormallySelected = Boolean(
        selection &&
        args.col >= selection.range.x &&
        args.col < selection.range.x + selection.range.width &&
        args.row >= selection.range.y &&
        args.row < selection.range.y + selection.range.height,
      );
      if (
        (!record?.shipment_group_id || !shipmentFields.has(key)) &&
        isNormallySelected
      ) {
        args.ctx.save();
      ...
      const hasSharedBoundaryAbove = Boolean(
        record?.shipment_group_id &&
        rows[args.row - 1]?.shipment_group_id === record.shipment_group_id,
      );
      ...
      if (
        hasSharedBoundaryAbove &&
        (!shipmentFields.has(key) || !record?.shipment_group_id)
      ) {
        args.ctx.save();
      ...
      const top =
        args.rect.y -
        Array.from({ length: args.row - first }, (_, index) =>
          rowHeightForIndex(first + index),
        ).reduce((sum, rowHeight) => sum + rowHeight, 0);
      ...
      if (display) {
        ...
        const maxWidth = Math.max(
          0,
          args.rect.width - args.theme.cellHorizontalPadding * 2,
        );
        const lines: string[] = [];
  ...
  const mergedRowTheme = useCallback(
    (row: number) => {
      ...
      const record = rows[row];
      return record?.shipment_group_id &&
        rows[row - 1]?.shipment_group_id === record.shipment_group_id
  ...
  const mergedEditorHeight = useMemo(() => {
    ...
    return Array.from({ length: range.height }, (_, index) =>
      rowHeightForIndex(row + index),
  ...
  const onGridKeyDown = useCallback(
    (event: { key: string; cancel: () => void }) => {
      ...
      setGridSelection({
        current: {
          cell: [col, targetRow],
          range: { x: col, y: targetRow, width: 1, height: 1 },
          rangeStack: [],
        },
  ...
  const drawCenteredHeader = useCallback(
    (args: {
      ctx: CanvasRenderingContext2D;
      column: GridColumn;
      theme: { bgHeader: string; textHeader: string; headerFontStyle: string };
      rect: { x: number; y: number; width: number; height: number };
    }) => {
      const { ctx, rect, theme } = args;
      const field = String(args.column.id ?? "");
      const isShipperColumn =
        shipperEditableFields.has(field) || shipperFormulaFields.has(field);
      ctx.save();
    ...
    async ([col, row]: Item, cell: GridCell) => {
      ...
      if (!record) {
        ...
        const response = await fetch("/api/shipper/spreadsheet", {
          ...
          body: JSON.stringify({
            ...
            rowType: "item",
            values: { [key]: value },
            trailingBlankCount: Math.max(0, row - rows.length),
          }),
        });
        const result = await response.json();
        if (!response.ok)
          return setError(result.error ?? "Could not create row");
      ...
      if (record.shipment_group_id && shipmentInputFields.includes(key)) {
        ...
        const response = await fetch("/api/shipper/spreadsheet", {
          ...
          body: JSON.stringify({
            shipperId,
            operation: "update-shared",
            rowIds: groupRows.map((item) => item.id),
            values: { [key]: value },
          }),
        });
        const result = await response.json();
        if (!response.ok) {
          setError(result.error ?? "Could not update shipment field");
          void load();
        }
        return;
      }
          ...
          setRows((current) =>
            current.map((row) =>
              updates.has(row.id)
                ? {
                    ...row,
                    values: fullRowClears.has(row.id)
                      ? {}
                      : { ...row.values, ...updates.get(row.id) },
                  }
                : row,
            ),
          );
        });
        updates.forEach((values, rowId) =>
          queueSave(
            rowId,
            fullRowClears.has(rowId) ? {} : values,
            fullRowClears.has(rowId),
          ),
        );
      }
      return true;
  ...
  const height = useMemo(
    () =>
      Math.max(
        (rows.length + 12) * 34,
        viewportHeight - (mode === "internal" ? 190 : 155),
      ),
    [mode, rows.length, viewportHeight],
  );
  ...
  const fillSelection = async (color: string | null) => {
    ...
    const rowIndexes = selectedRows.length
      ? selectedRows
      : Array.from({ length: range!.height }, (_, index) => range!.y + index);
    const columnIndexes = selectedRows.length
    ...
    for (const rowIndex of rowIndexes) {
      ...
      for (const colIndex of columnIndexes) {
        ...
        if (
          !field ||
          field === "__lock" ||
          (mode === "shipper" && !shipperEditableFields.has(field))
        )
          continue;
    ...
    flushSync(() =>
      setRows((current) =>
        current.map((row) =>
          fillsByRow.has(row.id)
            ? { ...row, cell_fills: fillsByRow.get(row.id)! }
            : row,
        ),
      ),
    );
    const results = await Promise.all(
      [...fillsByRow].map(async ([rowId, cellFills]) => {
        const response = await fetch("/api/shipper/spreadsheet", {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ shipperId, rowId, cellFills }),
        });
        return { response, result: await response.json() };
      }),
    );
  ...
  const pasteFromClipboard = async () => {
    ...
    try {
      ...
      setError(
        "Could not read the clipboard. Allow clipboard access, then try again.",
      );
    }
  };
  const insertRow = async (placement: "above" | "below") => {
    ...
    const response = await fetch("/api/shipper/spreadsheet", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ shipperId, referenceRowId: target.id, placement }),
    });
    const result = await response.json();
  ...
  const uploadRemarkImage = async (
    file: File,
    targetOverride?: { rowId: string; field: string },
  ) => {
    const target = targetOverride ?? attachmentTarget;
    const row = target
      ? rows.find((item) => item.id === target.rowId)
      : undefined;
      ...
      flushSync(() =>
        setRows((currentRows) =>
          currentRows.map((item) =>
            item.id === row.id
              ? { ...item, values: { ...item.values, [target.field]: next } }
              : item,
          ),
        ),
      );
  ...
  const removeRemarkImage = (row: Row, field: string, url: string) => {
    ...
    const next = String(row.values[field] ?? "")
      .replace(marker, "")
      .replace(/\n{3,}/g, "\n\n")
      .trim();
    localEditRevision.current += 1;
    flushSync(() =>
      setRows((currentRows) =>
        currentRows.map((item) =>
          item.id === row.id
            ? { ...item, values: { ...item.values, [field]: next } }
            : item,
        ),
      ),
    );
    queueSave(row.id, { [field]: next });
    void fetch("/api/shipper/delete-image", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  const selectedRowIndexes = () => {
    ...
    if (selectedRows.length)
      return [...new Set(selectedRows)].sort((left, right) => left - right);
    const range = gridSelection?.current?.range;
  ...
  const beginMerge = () => {
    ...
    if (
      rowIndexes.some(
        (index, position) =>
          position > 0 && index !== rowIndexes[position - 1] + 1,
      )
    ) {
    ...
    const incompleteExistingGroup = selectedRows.some(
      (row) =>
    ...
    if (incompleteExistingGroup) {
      toast.error(
        "Select every row in an existing shipment before changing its grouping.",
      );
      return;
    }
    const conflicts = shipmentInputFields.flatMap((field) => {
      const options = [
        ...new Set(
          selectedRows
            .map((row) => valueKey(row.values[field]))
            .filter(Boolean),
        ),
      ];
    ...
    const choices = Object.fromEntries(
      conflicts.map(({ field, options }) => [field, options[0]]),
    );
    if (!conflicts.length) {
      void applyMerge(rowIndexes, choices);
      return;
    }
  ...
  const beginUnmerge = async () => {
    ...
    if (
      !groupId ||
      selectedRows.some((row) => row.shipment_group_id !== groupId)
    )
      return;
    if (
      rows.some(
        (row) =>
          row.shipment_group_id === groupId &&
  ...
  const selectionAlreadyMerged = (() => {
    ...
    return (
      ...
      Boolean(groupId) &&
      selectedRows.every((row) => row.shipment_group_id === groupId)
    );
  })();
  const unmergeableSelection = (() => {
    ...
    return (
      ...
      selectedRows.every((row) => row.shipment_group_id === groupId) &&
      rows.every(
        (row) => row.shipment_group_id !== groupId || selectedIds.has(row.id),
      )
    );
  })();
  ...
  const contextField = contextMenu
    ...
    : "";
  const contextImages =
    contextRow && remarksFields.has(contextField)
      ? remarkImages(contextRow.values[contextField])
      : [];
  const canEditContextRemarks = Boolean(
    contextRow &&
    remarksFields.has(contextField) &&
    !contextRow.is_locked &&
    (mode !== "shipper" || shipperEditableFields.has(contextField)),
  );
  const dialogRow = remarkDialog
    ? rows.find((row) => row.id === remarkDialog.rowId)
    : undefined;
  const dialogImages =
    dialogRow && remarkDialog
      ? remarkImages(dialogRow.values[remarkDialog.field])
      : [];
  const selectedCell =
    ...
    gridSelection.current.range.height === 1
      ...
      : undefined;
  const selectedRemarkField = selectedCell
    ? String(columns[selectedCell[0]]?.id ?? "")
    : "";
  const selectedRemarkRow = selectedCell ? rows[selectedCell[1]] : undefined;
  const isSelectedRemarkCell = Boolean(
    selectedRemarkRow && remarksFields.has(selectedRemarkField),
  );
  const canAttachSelectedRemark = Boolean(
    isSelectedRemarkCell &&
    selectedRemarkRow &&
    !selectedRemarkRow.is_locked &&
    (mode !== "shipper" || shipperEditableFields.has(selectedRemarkField)),
  );
  const eligibleDropTargetAt = (
    clientX: number,
    clientY: number,
    dataTransfer: DataTransfer | null,
  ) => {
    const hasImage =
      [...(dataTransfer?.files ?? [])].some((file) =>
        file.type.startsWith("image/"),
      ) ||
      [...(dataTransfer?.items ?? [])].some(
        (item) => item.kind === "file" && item.type.startsWith("image/"),
      );
    if (!hasImage) return null;
    for (let row = 0; row < rows.length + 12; row += 1) {
      for (let col = 0; col < columns.length; col += 1) {
        ...
        if (
          ...
          continue;
        const field = String(columns[col]?.id ?? "");
        const record = rows[row];
        return record &&
          remarksFields.has(field) &&
          !record.is_locked &&
          (mode !== "shipper" || shipperEditableFields.has(field))
          ? { row, col, record, field }
          : null;
      }
    }
    return null;
  };
  // Glide still receives drag callbacks, but this is intentionally a no-op
  // until we revisit a reliable in-cell hover treatment.
  const showDropTarget = (next: Item | null) => {
    void next;
  };
  return (
    <section className="board-typography space-y-2 p-4">
      <div className="flex items-center gap-2">
        ...
        {gridSelection?.current && (
          <button
            type="button"
            onClick={() => setIsFillPaletteOpen((open) => !open)}
            className="flex items-center gap-1 rounded border border-slate-300 px-2 py-1.5 text-xs text-slate-700"
          >
            <PaintBucket size={15} />
            Fill colour
          </button>
        ...
        {isSelectedRemarkCell && (
          <button
            type="button"
            disabled={!canAttachSelectedRemark || uploadingAttachment}
            onClick={() => {
              if (selectedRemarkRow)
                openAttachmentPicker(selectedRemarkRow, selectedRemarkField);
            }}
            className="flex items-center gap-1 rounded border border-slate-300 px-2 py-1.5 text-xs text-slate-700 disabled:cursor-not-allowed disabled:opacity-40"
          >
            <ImagePlus size={15} />
            Attach image
          </button>
          ...
          <div className="flex flex-wrap gap-1 py-1">
            {fillColors.map((color) => (
              <button
                key={color}
                type="button"
                onClick={() => void fillSelection(color)}
            ...
            <button
              type="button"
              onClick={() => void fillSelection(null)}
              className="rounded border border-slate-300 px-2 text-[10px] text-slate-600"
            >
              Clear
            </button>
          </div>
      ...
      <div
        ...
        onDropCapture={(event) => {
          const target = eligibleDropTargetAt(
            event.clientX,
            event.clientY,
            event.dataTransfer,
          );
          ...
          if (image)
            void uploadRemarkImage(image, {
              rowId: target.record.id,
              field: target.field,
            });
        }}
      >
        <input
          ...
          onChange={(event) => {
            ...
            if (file) void uploadRemarkImage(file);
          }}
        />
        <DataEditor
          ref={gridRef}
          width="100%"
          height={height}
          rowHeight={rowHeightForIndex}
          columns={columns}
          rows={rows.length + 12}
          theme={{
            borderColor: "#94a3b8",
            horizontalBorderColor: "#a8b4c1",
            baseFontStyle: "14px Figtree, Roboto, sans-serif",
            headerFontStyle: "14px Figtree, Roboto, sans-serif",
          }}
          getRowThemeOverride={mergedRowTheme}
          getCellContent={getCellContent}
          drawCell={drawMergedShipmentCell}
          drawHeader={drawCenteredHeader}
          drawFocusRing={false}
          onKeyDown={onGridKeyDown}
          onCellEdited={onCellEdited}
          onCellsEdited={onCellsEdited}
          validateCell={([col], next) => {
            if (next.kind !== GridCellKind.Text) return true;
            const message = validationMessage(
              String(columns[col]?.id ?? ""),
              next.data,
            );
            if (message)
              toast.error("Invalid cell value", { description: message });
            return !message;
          }}
          provideEditor={(cell) => {
            if (cell.kind !== GridCellKind.Text) return undefined;
            const field = (cell as GridCell & { field?: string }).field ?? "";
            const styleOverride = mergedEditorHeight
              ? { height: `${mergedEditorHeight}px` }
              : undefined;
            if (remarksFields.has(field))
              return {
            ...
            return { editor: TextEditor, disablePadding: true, styleOverride };
          }}
          onCellClicked={([col, row]) => {
            setContextMenu(null);
            const record = rows[row];
            setSelectedRowId(record?.id ?? null);
            if (col === 0 && record) void toggleRowLock(record);
          }}
          onCellContextMenu={([col, row], event) => {
            event.preventDefault();
            const current = gridSelection?.current?.range;
            const withinCurrentSelection =
              Boolean(gridSelection?.rows.hasIndex(row)) ||
              Boolean(
                current &&
                col >= current.x &&
                col < current.x + current.width &&
                row >= current.y &&
            ...
            setSelectedRowId(rows[row]?.id ?? null);
            const pointer = contextPointer.current;
            setContextMenu({
              x: pointer?.x ?? event.bounds.x,
              y: pointer?.y ?? event.bounds.y,
              row,
              col,
            });
          }}
          onDragOverCell={([col, row], dataTransfer) => {
            const field = String(columns[col]?.id ?? "");
            const record = rows[row];
            const hasImage = [...(dataTransfer?.files ?? [])].some((file) =>
              file.type.startsWith("image/"),
            );
            ...
            showDropTarget(hasImage && editable ? [col, row] : null);
          }}
          onDragLeave={() => showDropTarget(null)}
          onDrop={([col, row], dataTransfer) => {
            showDropTarget(null);
            const field = String(columns[col]?.id ?? "");
            const record = rows[row];
            const image = [...(dataTransfer?.files ?? [])].find((file) =>
              file.type.startsWith("image/"),
            );
            if (
              !image ||
              ...
              return;
            void uploadRemarkImage(image, { rowId: record.id, field });
          }}
        ...
        {contextMenu && (
          ...
          >
            ...
            <button
              ...
              className="flex w-full items-center gap-1 px-3 py-1.5 text-left text-xs hover:bg-slate-100"
            >
              <PaintBucket size={14} />
              Colour fill
            </button>
            {canEditContextRemarks && (
              <button
                ...
                onClick={() => {
                  if (contextRow)
                    openAttachmentPicker(contextRow, contextField);
                  setContextMenu(null);
                }}
                className="flex w-full items-center gap-1 px-3 py-1.5 text-left text-xs hover:bg-slate-100 disabled:text-slate-300"
              >
                <ImagePlus size={14} />
                Attach image
              </button>
            )}
            {contextImages.length > 0 && contextRow && (
              <button
                type="button"
                onClick={() => {
                  setRemarkDialog({
                ...
                className="block w-full px-3 py-1.5 text-left text-xs hover:bg-slate-100"
              >
                View images ({contextImages.length})
              </button>
            )}
            {mode !== "shipper" && mergeableSelection && (
              <button
                type="button"
                disabled={selectionAlreadyMerged}
                onClick={() => {
                  beginMerge();
                  setContextMenu(null);
                }}
                className="block w-full px-3 py-1.5 text-left text-xs hover:bg-slate-100 disabled:text-slate-300 disabled:hover:bg-transparent"
              >
                Merge selected rows
              </button>
            )}
            {mode !== "shipper" && unmergeableSelection && (
              <button
                type="button"
                onClick={() => {
                  void beginUnmerge();
                ...
                className="block w-full px-3 py-1.5 text-left text-xs hover:bg-slate-100"
              >
                Unmerge selected shipment
              </button>
            )}
            {rows[contextMenu.row] && (
              <button
                type="button"
                disabled={!hasRowContent(rows[contextMenu.row])}
                onClick={() => {
                  void toggleRowLock(rows[contextMenu.row]);
                  setContextMenu(null);
                }}
                className="block w-full px-3 py-1.5 text-left text-xs hover:bg-slate-100 disabled:text-slate-300"
              >
                {rows[contextMenu.row].is_locked ? "Unlock row" : "Lock row"}
              </button>
            )}
          </div>
        )}
        {mergeDialog && (
          <div className="absolute inset-0 z-40 flex items-start justify-center bg-slate-900/20 pt-12">
            <div
              role="dialog"
              aria-modal="true"
            ...
            >
              <h2 className="text-base font-semibold text-slate-900">
                Resolve shipment field conflicts
              </h2>
              <p className="mt-1 text-sm text-slate-600">
                Merged shipment columns need one shared value. Choose the value
                to keep for each conflicting field; it will be applied to all
                selected rows.
              </p>
              <div className="mt-4 max-h-72 space-y-3 overflow-y-auto">
                {mergeDialog.conflicts.map(({ field, options }) => (
                  <label key={field} className="block text-sm text-slate-700">
                    <span className="mb-1 block font-medium">
                      {columns.find((column) => String(column.id) === field)
                        ?.title ?? field}
                    </span>
                    <select
                      value={mergeDialog.choices[field]}
                      onChange={(event) =>
              ...
              <div className="mt-5 flex justify-end gap-2">
                ...
                <button
                  type="button"
                  onClick={() =>
                    void applyMerge(mergeDialog.rowIndexes, mergeDialog.choices)
                  }
            ...
            >
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-600">
                    Attachments for this spreadsheet row.
                  </p>
                </div>
                <button
                  type="button"
              ...
              <div className="mt-4 grid max-h-[60vh] grid-cols-2 gap-4 overflow-y-auto sm:grid-cols-3">
                {dialogImages.map((url) => (
                  ...
                  >
                    ...
                    {!dialogRow.is_locked &&
                      (mode !== "shipper" ||
                        shipperEditableFields.has(remarkDialog.field)) && (
                        <button
                          type="button"
                          onClick={() =>
                            removeRemarkImage(
                          ...
                          className="flex w-full items-center justify-center gap-1 border-t border-slate-200 px-2 py-2 text-xs text-red-700 hover:bg-red-50"
                        >
                          <Trash2 size={13} />
                          Remove
                        </button>
