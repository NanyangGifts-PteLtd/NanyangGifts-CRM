...
const columns: Column[] = [
  ...
  { key: "chargeable_weight_kg", label: "计费重量 (KG)", scope: "shipment", input: "number" },
  { key: "destination", label: "目的地", scope: "shipment" },
  { key: "freight_unit_price", label: "单价", scope: "shipment", input: "number", width: 130 },
  ...
  { key: "logistics_remarks", label: "备注", scope: "shipment", input: "textarea", width: 220 },
  { key: "ic", label: "谁下单 / I/C", scope: "shipment" },
  { key: "date_of_submission", label: "提供资料日期", scope: "shipment", input: "date" },
  ...
  { key: "delivery_info", label: "地址 / Address", scope: "shipment", input: "textarea", width: 280 },
  { key: "quantity", label: "数量 / Qty", scope: "item", input: "number" },
  { key: "unit_price", label: "单价 / Unit Price", scope: "item", input: "number", width: 150 },
  { key: "declared_value", label: "货值 / Value", scope: "item", calculated: true },
  { key: "sea_or_air", label: "海运、空运 / Sea or Air?", scope: "shipment", input: "select-air" },
  { key: "tax_refund", label: "退税?", scope: "shipment", input: "select-tax" },
  { key: "remarks", label: "备注 / Remarks", scope: "item", input: "textarea", width: 220 },
  { key: "samples_by_air", label: "发样品空运 / Samples by air", scope: "item" },
  { key: "samples_by_sea", label: "发样品海运 / Samples by sea", scope: "item" },
...
function EditableCell({
  ...
  if (column.calculated) {
    return <div className="flex min-h-[42px] items-center justify-center px-2">{text(value)}</div>;
  }

  if (column.input === "select-air") {
    return (
      <select value={draft} onChange={(event) => onSave(event.target.value)} className="min-h-[42px] w-full border-0 bg-transparent px-2 text-center outline-none focus:bg-blue-50">
        <option value="" />
...
function ShipmentRemarkCell({ value, shipmentId, shipperId, itemId, onSave }: { value: unknown; shipmentId: string; shipperId: string | null; itemId?: string; onSave: (value: string) => void }) {
  ...
  const upload = async (file: File) => {
    ...
    setUploading(true);
    try {
      const form = new FormData(); form.append("file", file); if (shipperId) form.append("shipperId", shipperId);
      if (itemId) form.append("shipmentItemId", itemId); else form.append("shipmentId", shipmentId);
    ...
    } catch (error) { window.alert(error instanceof Error ? error.message : "Image upload failed"); } finally { setUploading(false); }
  };
  const urls = [...draft.matchAll(/\[\[shipper-image:(https?:\/\/[^\]]+)\]\]/g)].map(match => match[1]);
  const plainText = draft.replace(/\[\[shipper-image:https?:\/\/[^\]]+\]\]/g, "").replace(/\n{3,}/g, "\n\n").trim();
  const setPlainText = (nextText: string) => setDraft(`${nextText.trim()}${nextText.trim() && urls.length ? "\n\n" : ""}${urls.map(url => `[[shipper-image:${url}]]`).join("\n")}`);
  const remove = async (url: string) => { setUploading(true); try { const response = await fetch("/api/shipper/delete-image", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ url }) }); const result = await response.json(); if (!response.ok) throw new Error(result.error || "Could not remove image"); const next = draft.replace(`[[shipper-image:${url}]]`, "").replace(/\n{3,}/g, "\n\n").trim(); setDraft(next); onSave(next); } catch (error) { window.alert(error instanceof Error ? error.message : "Could not remove image"); } finally { setUploading(false); } };
  return <div className="min-h-[56px]" onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); const file = Array.from(event.dataTransfer.files).find(candidate => candidate.type.startsWith("image/")); if (file) void upload(file); }}><textarea spellCheck={false} value={plainText} rows={2} onChange={event => setPlainText(event.target.value)} onBlur={() => onSave(draft)} className="block min-h-[56px] w-full resize-y border-0 bg-transparent px-3 py-2 text-center outline-none focus:bg-blue-50" />{urls.map(url => <div key={url} className="relative mx-auto mb-2 w-fit"><a href={url} target="_blank" rel="noreferrer"><img src={url} alt="Remark attachment" className="max-h-32 max-w-[180px] rounded border" /></a><button type="button" disabled={uploading} onClick={() => void remove(url)} className="absolute right-1 top-1 rounded bg-white/90 p-1 text-red-600 shadow"><Trash2 size={13} /></button></div>)}<label className="flex cursor-pointer items-center justify-center gap-1 pb-2 text-xs text-slate-500"><ImagePlus size={13} />{uploading ? "Uploading…" : "Drop or attach image"}<input type="file" accept="image/*" className="sr-only" disabled={uploading} onChange={event => { const file = event.target.files?.[0]; if (file) void upload(file); event.target.value = ""; }} /></label></div>;
}

export function ShipmentGrid({ shipments, mode = "pm" }: ShipmentGridProps) {
  const [gridShipments, setGridShipments] = useState(shipments);
  const [selectedCells, setSelectedCells] = useState<Array<{ id: string; scope: "shipment" | "item"; field: string }>>([]);
  ...
  const saveCell = async (shipmentId: string, itemId: string | undefined, field: string, value: string) => {
    const isShipment = !itemId;
    const targetId = isShipment ? shipmentId : itemId;
    const targetKey = `${targetId}:${field}`;
    setSavingCell(targetKey);

    setGridShipments((current) => current.map((shipment) => {
      if (shipment.id !== shipmentId) return shipment;
      if (isShipment) return { ...shipment, [field]: value };
      return { ...shipment, items: shipment.items.map((item) => item.id === itemId ? { ...item, [field]: value } : item) };
    }));
    ...
    try {
      const response = await fetch("/api/shipper/shipments", {
        ...
        body: JSON.stringify(isShipment ? { shipmentId, field, value } : { itemId, field, value }),
      });
      ...
      setGridShipments((current) => current.map((shipment) => {
        ...
        return { ...shipment, items: shipment.items.map((item) => item.id === itemId ? { ...item, ...result.values } : item) };
      }));
    } catch (error) {
      console.error(error);
      window.alert(error instanceof Error ? error.message : "Could not save shipment cell.");
  ...
  const fillSelectedCell = (color: string) => {
    selectedCells.forEach((selectedCell) => {
      const shipment = gridShipments.find((item) => item.id === selectedCell.id || item.items.some((line) => line.id === selectedCell.id));
      if (!shipment) return;
      const item = selectedCell.scope === "item" ? shipment.items.find((line) => line.id === selectedCell.id) : undefined;
    ...
    <div className="w-full">
      <div className="mb-1 flex h-10 max-w-full items-center gap-2 rounded-md border border-slate-300 bg-white px-2 shadow-sm">
        {paletteOpen && selectedCells.length ? (
          <>
            <span className="shrink-0 text-xs font-medium text-slate-500">Fill</span>
            <div className="flex min-w-0 flex-1 gap-1 overflow-x-auto py-1">
              {fillColors.map((color) => <button key={color} type="button" onClick={() => fillSelectedCell(color)} className="h-5 w-5 shrink-0 rounded border border-slate-300" style={{ backgroundColor: color }} title="Fill selected cell" />)}
              <button type="button" onClick={() => fillSelectedCell("")} className="shrink-0 rounded border border-slate-300 px-2 text-[10px] text-slate-600">Clear</button>
            </div>
            <button type="button" onClick={() => setPaletteOpen(false)} className="rounded p-1 text-slate-400 hover:bg-slate-100"><X size={14} /></button>
          </>
        ) : (
          <button type="button" disabled={!selectedCells.length} onClick={() => setPaletteOpen(true)} className="inline-flex items-center gap-1 rounded px-2 py-1 text-xs font-medium text-slate-600 hover:bg-slate-100 disabled:opacity-40"><PaintBucket size={15} /> Fill colour</button>
        )}
      </div>
