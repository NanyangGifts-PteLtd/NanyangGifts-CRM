...
export function ShipperMasterSheets({ shippers, rows, stagingRows, shipments }: { shippers: Shipper[]; rows: ShipperRow[]; stagingRows: ShipperStagingRow[]; shipments: ShipmentRecord[] }) {
    ...
    return <div className="flex min-h-0 flex-1 flex-col rounded-md border border-slate-300 bg-white shadow-sm">
        <div className="flex items-center justify-between gap-3 border-b border-slate-200 px-4 py-2.5">
            ...
            <div className="flex gap-2"><button onClick={() => setView('shipments')} className={`rounded px-2 py-1 text-xs ${view === 'shipments' ? 'bg-sky-600 text-white' : 'border text-slate-600'}`}>Shipments</button><button onClick={() => setView('legacy')} className={`rounded px-2 py-1 text-xs ${view === 'legacy' ? 'bg-sky-600 text-white' : 'border text-slate-600'}`}>Legacy grid</button>{activeShipper.website_url ? <a href={activeShipper.website_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 rounded border border-sky-200 bg-sky-50 px-2.5 py-1.5 text-xs font-medium text-sky-700 hover:bg-sky-100"><ExternalLink size={13} /> Open shipper website</a> : null}</div>
        </div>
        <div className="min-h-0 flex-1 overflow-auto">
            {view === 'shipments' ? <><div className="p-4"><ShipmentGrid shipments={shipments.filter((shipment) => shipment.shipper_id === activeShipper.id)} /></div><ShipperStagingTable key={activeShipper.id} shipper={activeShipper} initialRows={stagedRows.filter((row) => row.shipper_id === activeShipper.id)} onRowsChange={(next) => setStagedRows((current) => [...current.filter((row) => row.shipper_id !== activeShipper.id), ...next])} onPushed={() => router.refresh()} onStandalonePushed={() => router.refresh()} /></> : <><ShipperGrid rows={gridRows.filter((row) => row.shipper_id === activeShipper.id)} mode="pm" /><ShipperStagingTable key={activeShipper.id} shipper={activeShipper} initialRows={stagedRows.filter((row) => row.shipper_id === activeShipper.id)} onRowsChange={(next) => setStagedRows((current) => [...current.filter((row) => row.shipper_id !== activeShipper.id), ...next])} onPushed={(row) => setGridRows((current) => { const exists = current.some((item) => item.subitem_id === row.subitem_id); return exists ? current.map((item) => item.subitem_id === row.subitem_id ? row : item) : [...current, row]; })} /></>}
        </div>
