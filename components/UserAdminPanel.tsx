...
export function UserAdminPanel({ profiles }: UserAdminPanelProps) {
  ...
  const [pendingSuspension, setPendingSuspension] = useState<Profile | null>(null);
  ...
  const sortedProfiles = useMemo(() => {
    ...
    return [...profiles].sort((a, b) => {
      const rankA = roleOrder.indexOf((a.role || '').toLowerCase() as typeof roleOrder[number]);
      const rankB = roleOrder.indexOf((b.role || '').toLowerCase() as typeof roleOrder[number]);
      if ((rankA === -1 ? roleOrder.length : rankA) !== (rankB === -1 ? roleOrder.length : rankB)) return (rankA === -1 ? roleOrder.length : rankA) - (rankB === -1 ? roleOrder.length : rankB);
  ...
  const saveSuspension = async () => {
    ...
    setError(null);
    try {
      const response = await fetch('/api/admin/users', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: pendingSuspension.id, suspended: nextSuspended }) });
      const result = await response.json() as { error?: string };
      if (!response.ok) throw new Error(result.error || 'Unable to update account status.');
      setSuspendedById((previous) => ({ ...previous, [pendingSuspension.id]: nextSuspended }));
      setMessage(`${pendingSuspension.full_name || pendingSuspension.email || 'User'} has been ${nextSuspended ? 'suspended' : 'restored'}.`);
      setPendingSuspension(null);
    } catch (suspensionError) {
      setError(suspensionError instanceof Error ? suspensionError.message : 'Unable to update account status.');
    } finally {
    ...
    <div className="flex h-full flex-col gap-4 bg-[#f8fafc] p-4">
      ...
      <div className="mx-auto w-full max-w-5xl rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-200 px-5 py-4">
          <h3 className="text-sm font-semibold text-slate-900">Current team &amp; user suspension</h3>
          <p className="mt-1 text-xs text-slate-500">Suspend non-director accounts to prevent future sign-ins, or restore access when needed.</p>
        </div>
          ...
          <table className="min-w-full border-collapse text-sm">
            <thead className="bg-slate-50 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">
              <tr>
                ...
                <th className="border-b border-slate-200 px-5 py-3">Account status</th>
            ...
            <tbody>
              {sortedProfiles.map((profile) => (
                <tr key={profile.id} className="border-b border-slate-100 last:border-b-0">
                  <td className="px-5 py-3 font-medium text-slate-800">{profile.full_name || 'Unnamed user'}</td>
                  <td className="px-5 py-3 text-slate-700">{profile.email || '-'}</td>
                  <td className="px-5 py-3 text-slate-700">{profile.role || 'unassigned'}</td>
                  <td className="px-5 py-3"><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${suspendedById[profile.id] ? 'bg-red-50 text-red-700' : 'bg-emerald-50 text-emerald-700'}`}>{suspendedById[profile.id] ? 'Suspended' : 'Active'}</span></td>
                  <td className="px-5 py-3 text-right">{profile.role?.toLowerCase() !== 'director' && <button onClick={() => setPendingSuspension(profile)} className={`inline-flex items-center gap-1 rounded-md border px-2.5 py-1.5 text-xs font-medium ${suspendedById[profile.id] ? 'border-emerald-200 text-emerald-700 hover:bg-emerald-50' : 'border-red-200 text-red-700 hover:bg-red-50'}`}>{suspendedById[profile.id] ? <RotateCcw size={13} /> : <Ban size={13} />}{suspendedById[profile.id] ? 'Restore account' : 'Suspend account'}</button>}</td>
                </tr>
              ))}
              {profiles.length === 0 && <tr><td colSpan={5} className="px-5 py-8 text-center text-sm text-slate-500">No team members found.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
      {pendingSuspension && <div className="fixed inset-0 z-[220] flex items-center justify-center bg-slate-950/40 p-4"><div className="w-full max-w-md rounded-xl bg-white p-5 shadow-2xl"><h3 className="text-base font-semibold text-slate-900">{suspendedById[pendingSuspension.id] ? 'Restore account?' : 'Suspend account?'}</h3><p className="mt-2 text-sm text-slate-600">{suspendedById[pendingSuspension.id] ? `${pendingSuspension.full_name || pendingSuspension.email || 'This user'} will be able to sign in again.` : `${pendingSuspension.full_name || pendingSuspension.email || 'This user'} will not be able to sign in until their account is restored.`}</p><div className="mt-5 flex justify-end gap-2"><button disabled={isSavingSuspension} onClick={() => setPendingSuspension(null)} className="rounded-md border border-slate-200 px-3 py-2 text-sm text-slate-600">Cancel</button><button disabled={isSavingSuspension} onClick={() => void saveSuspension()} className={`rounded-md px-3 py-2 text-sm font-medium text-white disabled:opacity-60 ${suspendedById[pendingSuspension.id] ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-red-600 hover:bg-red-700'}`}>{isSavingSuspension ? 'Saving...' : suspendedById[pendingSuspension.id] ? 'Restore account' : 'Suspend account'}</button></div></div></div>}
    </div>
  );
}
