            ...
            {filtered.length ? (
              filtered.map((supplier) => (
                ...
                >
                  ...
                  <button type="button" aria-busy={pending === `open:${supplier.id}`} onClick={() => void open(supplier.id).catch((error) => toast.error(error.message))} disabled={pending === `open:${supplier.id}`} className="grid min-w-0 flex-1 grid-cols-[minmax(180px,1fr)_minmax(220px,1.2fr)_minmax(360px,2fr)] items-center gap-5 text-left disabled:cursor-wait disabled:opacity-60">
                    <div className="min-w-0"><div className="flex items-center gap-2"><p className="truncate font-semibold">{supplier.name}</p>{pending === `open:${supplier.id}` && <LoaderCircle className="shrink-0 animate-spin text-amber-600" size={16} />}</div>{pending === `open:${supplier.id}` ? <p className="mt-0.5 text-xs text-slate-500">Opening profile...</p> : supplier.is_blacklisted && <p className="mt-0.5 text-xs font-semibold text-red-700">Blacklisted</p>}</div>
