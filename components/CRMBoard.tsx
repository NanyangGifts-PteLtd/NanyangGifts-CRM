...
const DEFAULT_BOARD_SORT: BoardSortSetting = {
  category: "client",
  column: "dateCreated",
  direction: "desc",
};
...
const CLIENT_HEADER_COLS: HeaderCol[] = [
  ...
  {
    key: "logoRequirementsFile",
    label: "Logo/Requirements File",
    width: 150,
    minWidth: 7,
  },
  {
...
export function CRMBoard({
  ...
  const [filterMode, setFilterMode] = useState<"quick" | "advanced">(
    "advanced",
  );
  const [advancedRules, setAdvancedRules] = useState<AdvancedFilterRule[]>([
    { id: "initial", column: "", condition: "", value: "" },
  ]);
  ...
  useEffect(() => {
    ...
    const selector = searchTarget.subitemId
      ? `[data-subitem-id="${searchTarget.subitemId}"]`
      : `[data-client-id="${searchTarget.clientId}"]`;
    let attempts = 0;
  ...
  const allExpanded =
    clients.length > 0 && clients.every((c) => expandedIdSet.has(c.id));
  ...
  useEffect(() => {
    ...
    const targetGroupId = clients.find(
      (client) => client.id === searchTarget.clientId,
    )?.groupId;
    setExpandedIds((current) =>
      current.includes(searchTarget.clientId)
        ? current
        : [...current, searchTarget.clientId],
    );

    if (targetGroupId) {
      setCollapsedGroups((current) =>
        current[targetGroupId]
          ? { ...current, [targetGroupId]: false }
          : current,
      );
    }
    ...
    if (searchTarget.kind === "timeline") {
      setClients((current) =>
        current.map((client) =>
          client.id !== searchTarget.clientId
            ? client
            : {
                ...client,
                subitems: client.subitems.map((subitem) =>
                  searchTarget.subitemId &&
                  subitem.id !== searchTarget.subitemId
                    ? subitem
                    : {
                        ...subitem,
                        showTimeline: true,
  ...
  const [pendingDeleteClientId, setPendingDeleteClientId] = useState<
    string | null
  >(null);
  const [pendingDeleteSubitem, setPendingDeleteSubitem] = useState<{
    clientId: string;
    subitemId: string;
  } | null>(null);
  ...
  useEffect(() => {
    if (!openClientId || !clients.some((client) => client.id === openClientId))
      return;
  ...
  const canEditClientRecord = useCallback(
    (clientId: string) => {
      ...
      return (
        !!client &&
        ((clientAssignees[clientId] ?? []).includes(currentUserId) ||
  ...
  const canEditSubitemRecord = useCallback(
    (clientId: string, subitemId: string) => {
      if (!currentUserId) return false;
      return (
        canEditClientRecord(clientId) ||
        (subitemAssignees[subitemId] ?? []).includes(currentUserId)
      );
    },
  ...
  const [localOverseasEntries, setLocalOverseasEntries] = useState<
    OptionEntry[]
  >([]);
  const [subitemStatusEntries, setSubitemStatusEntries] = useState<
    OptionEntry[]
  >([]);
  ...
  const subitemStatusOptionsForFilter = subitemStatusEntries.map(
    (e) => e.value,
  );
  const paymentOptionsForFilter = paymentEntries.map((e) => e.value);
  const paymentStatusOptionsForFilter = paymentStatusEntries.map(
    (e) => e.value,
  );
  ...
  const subProgressColors = Object.fromEntries(
    subitemSubprogressEntries.map((e) => [e.value, e.color]),
  );
  const subitemStatusColors = Object.fromEntries(
    subitemStatusEntries.map((e) => [e.value, e.color]),
  );
  const paymentColors = Object.fromEntries(
    paymentEntries.map((e) => [e.value, e.color]),
  );
  const paymentStatusColors = Object.fromEntries(
    paymentStatusEntries.map((e) => [e.value, e.color]),
  );
  const importanceColors = Object.fromEntries(
    importanceEntries.map((e) => [e.value, e.color]),
  );
  const progressColors = Object.fromEntries(
    progressEntries.map((e) => [e.value, e.color]),
  );
  const replyStatusColors = Object.fromEntries(
    replyStatusEntries.map((e) => [e.value, e.color]),
  );
  const channelColors = Object.fromEntries(
    channelEntries.map((e) => [e.value, e.color]),
  );
  ...
  const [dragOverSubitemClientId, setDragOverSubitemClientId] = useState<
    string | null
  >(null);
  const [subitemDropMarker, setSubitemDropMarker] = useState<{
    clientId: string;
    subitemId: string;
    edge: "top" | "bottom";
  } | null>(null);
  ...
  const [dragOverGroupEdge, setDragOverGroupEdge] = useState<
    "top" | "bottom" | null
  >(null);
  const [groupDragOverId, setGroupDragOverId] = useState<string | null>(null);
  const [groupDragOverEdge, setGroupDragOverEdge] = useState<
    "top" | "bottom" | null
  >(null);
  const [pendingCloseLead, setPendingCloseLead] = useState<{
    clientId: string;
  ...
  const [showRestoreArrangementConfirm, setShowRestoreArrangementConfirm] =
    useState(false);
  const [showRestoreSortingConfirm, setShowRestoreSortingConfirm] =
    useState(false);
  ...
  const [boardSort, setBoardSort] =
    useState<BoardSortSetting>(DEFAULT_BOARD_SORT);
  ...
  useEffect(() => {
    ...
    void supabase
      ...
      .then(({ data, error }) => {
        ...
        setSignedOcfCheck({
          loading: false,
          signedAt: error ? null : (data?.client_signed_at ?? null),
          error: Boolean(error),
        });
      });
    return () => {
  ...
  const hideColumn = useCallback(
    (key: string) => {
      ...
      notifyChange(
        "Column hidden",
        `${key.replace(/^[^:]+:/, "")} is hidden for your account.`,
      );
    },
    [notifyChange],
  );
  ...
  const setColumnVisibility = useCallback(
    (key: string, visible: boolean) => {
      ...
      notifyChange(
        visible ? "Column restored" : "Column hidden",
        visible
          ? `${key.replace(/^[^:]+:/, "")} is visible again.`
          : `${key.replace(/^[^:]+:/, "")} is hidden for your account.`,
      );
    },
    [notifyChange],
  ...
  useEffect(() => {
    if (!currentUserId || hiddenSettingsLoadedFor.current !== currentUserId)
      return;
  ...
  const setDragPreview = (
    ...
    source: HTMLElement,
    includeColumnCells = false,
  ) => {
    if (!event.dataTransfer) return;
    const bounds = source.getBoundingClientRect();
    const preview = includeColumnCells
    ...
    preview.style.height = includeColumnCells
      ? `${bounds.height + 56}px`
      : `${bounds.height}px`;
    ...
    if (includeColumnCells) {
      ...
      const groupRows = groupContainer
        ? Array.from(
            groupContainer.querySelectorAll<HTMLElement>("[data-client-row]"),
          )
        : [];
      const rowScope: ParentNode =
        groupRows.length > 0 && groupContainer ? groupContainer : document;
      const visibleCells = Array.from(
        rowScope.querySelectorAll<HTMLElement>("[data-client-row]"),
      )
        .flatMap((row) =>
          Array.from(row.querySelectorAll<HTMLElement>('[style*="order"]')),
        )
        .filter((cell) => {
          const cellBounds = cell.getBoundingClientRect();
          return (
            cellBounds.top >= bounds.bottom - 1 &&
            Math.abs(cellBounds.left + cellBounds.width / 2 - sourceCenter) <
    ...
    event.dataTransfer.setDragImage(
      preview,
      Math.min(bounds.width / 2, 90),
      Math.min(bounds.height / 2, includeColumnCells ? 28 : 16),
    );
    window.setTimeout(() => preview.remove(), 0);
  };

  const handleRestoreDefaults = useCallback(
    async (scope: ColumnScope) => {
      ...
      const clientMap = Object.fromEntries(
        CLIENT_HEADER_COLS.map((c) => [c.key, c.width]),
      );
      const subitemMap = Object.fromEntries(
        SUBITEM_COLS.map((c) => [c.key, c.width]),
      );
      const paymentMap = Object.fromEntries(
        PAYMENT_COLS.map((c) => [c.key, c.width]),
      );
      ...
      if (scope === "client" || scope === "all")
        try {
          localStorage.setItem(
            "colWidths:clients:local",
            JSON.stringify(clientMap),
          );
          localStorage.setItem(
            "colWidths:clients:local_owner",
            String(currentUserId ?? "anon"),
          );
        } catch {}
      if (scope === "subitem" || scope === "all")
        try {
          localStorage.setItem(
            "colWidths:subitems:local",
            JSON.stringify(subitemMap),
          );
          localStorage.setItem(
            "colWidths:payments:local",
            JSON.stringify(paymentMap),
          );
          localStorage.setItem(
            "colWidths:subitems:local_owner",
            String(currentUserId ?? "anon"),
          );
          localStorage.setItem(
            "colWidths:payments:local_owner",
            String(currentUserId ?? "anon"),
          );
        } catch {}
      if (currentUserId) {
      ...
      try {
        if (scope === "subitem" || scope === "all") {
          window.dispatchEvent(
            new CustomEvent("subitemColsChanged", { detail: subitemMap }),
          );
          window.dispatchEvent(
            new CustomEvent("paymentColsChanged", { detail: paymentMap }),
          );
        }
      } catch (e) {
        // ignore
      ...
      if (currentUserId) {
        try {
          ...
          const saves = [];
          if (scope === "client" || scope === "all")
            saves.push(saveUserSetting("colWidths:clients", clientMap));
          if (scope === "subitem" || scope === "all")
            saves.push(
      ...
      notifyChange(
        ...
        scope === "client"
          ? "Client column widths were reset."
          : scope === "subitem"
            ? "Subitem and payment column widths were reset."
            : "All column widths were reset.",
      );
    },
    [currentUserId, notifyChange],
  );

  const handleRestoreDefaultArrangement = useCallback(
    async (scope: ColumnScope) => {
      ...
      try {
        if (scope === "client" || scope === "all") {
          localStorage.setItem(
            "colOrder:clients:local",
            JSON.stringify(clientOrder),
          );
          localStorage.setItem(
            "colOrder:clients:local_owner",
            String(currentUserId ?? "anon"),
          );
        }
        if (scope === "subitem" || scope === "all") {
          localStorage.setItem(
            "colOrder:subitems:local",
            JSON.stringify(subitemOrder),
          );
          localStorage.setItem(
            "colOrder:payments:local",
            JSON.stringify(paymentOrder),
          );
          localStorage.setItem(
            "colOrder:subitems:local_owner",
            String(currentUserId ?? "anon"),
          );
          localStorage.setItem(
            "colOrder:payments:local_owner",
            String(currentUserId ?? "anon"),
          );
        }
        if (currentUserId) {
          if (scope === "client" || scope === "all")
            localStorage.setItem(
              `colOrder:clients:${currentUserId}`,
              JSON.stringify(clientOrder),
            );
          if (scope === "subitem" || scope === "all") {
            localStorage.setItem(
              `colOrder:subitems:${currentUserId}`,
              JSON.stringify(subitemOrder),
            );
            localStorage.setItem(
              `colOrder:payments:${currentUserId}`,
              JSON.stringify(paymentOrder),
            );
          }
        }
        if (scope === "subitem" || scope === "all") {
          window.dispatchEvent(
            new CustomEvent("subitemColsReordered", { detail: subitemOrder }),
          );
          window.dispatchEvent(
            new CustomEvent("paymentColsReordered", { detail: paymentOrder }),
          );
        }
        if (scope === "client" || scope === "all")
          window.dispatchEvent(
            new CustomEvent("clientColsReordered", { detail: clientOrder }),
          );
      } catch {}
      ...
      if (currentUserId) {
        try {
          ...
          const saves = [];
          if (scope === "client" || scope === "all")
            saves.push(saveUserSetting("colOrder:clients", clientOrder));
          if (scope === "subitem" || scope === "all")
            saves.push(
          ...
          console.warn(
            "Failed to persist restored default column arrangement",
            error,
          );
        }
      }
      notifyChange(
        "Column arrangement restored",
        scope === "client"
          ? "Client columns were reset."
          : scope === "subitem"
            ? "Subitem and payment columns were reset."
            : "All columns were reset.",
      );
    },
    [currentUserId, notifyChange],
  );
  ...
  const [showAddColModal, setShowAddColModal] = useState<
    "client" | "subitem" | null
  >(null);
  const [newColName, setNewColName] = useState("");
  const [newColType, setNewColType] = useState<"text" | "number" | "date">(
    "text",
  );
  ...
  const mergedHeaderCols = React.useMemo<HeaderCol[]>(() => {
    ...
    const fixedFront = headerCols.filter((c) =>
      ["selectCheckbox", "client"].includes(c.key),
    );
    const middle = [
      ...headerCols.filter(
        (c) =>
  ...
  const clientColumnOrderMap = React.useMemo<Record<string, number>>(() => {
    return Object.fromEntries(
      mergedHeaderCols.map((col, index) => [col.key, index]),
    );
  }, [mergedHeaderCols]);
  ...
  const activeClientColumnOrderMap = React.useMemo<Record<string, number>>(
    () =>
      Object.fromEntries(
        activeClientHeaderCols.map((col, index) => [col.key, index]),
      ),
  ...
  const visibleClientCustomCols = React.useMemo(
    () =>
      clientCustomCols.filter(
        (col) => !hiddenColumnKeys.has(`client:custom:${col.id}`),
      ),
  ...
  const reorderClientColumns = useCallback(
    (draggedKey: string, targetKey: string) => {
      ...
      const fixedFront = mergedHeaderCols.filter((col) =>
        ["selectCheckbox", "client"].includes(col.key),
      );
      const fixedEnd = mergedHeaderCols.filter((col) =>
        ["addClientCol", "empty"].includes(col.key),
      );
      ...
      try {
        ...
        window.dispatchEvent(
          new CustomEvent("clientColsReordered", { detail: order }),
        );
      } catch {}
      if (currentUserId)
        void import("@/lib/user-settings")
          .then(({ saveUserSetting }) =>
            saveUserSetting("colOrder:clients", order),
          )
          .catch((error) =>
            console.warn("Failed to save client column arrangement", error),
          );
      notifyChange(
        "Column arrangement saved",
    ...
    () => [
      {
        ...
        columns: mergedHeaderCols
          ...
          .map((col) => ({ key: `client:${col.key}`, label: col.label })),
      },
      {
        label: "Subitem columns",
        columns: [
          ...SUBITEM_COLS.filter((col) => col.key !== "name").map((col) => ({
            key: `subitem:${col.key}`,
            label: col.label,
          })),
          ...subitemCustomCols.map((col) => ({
            key: `subitem:custom:${col.id}`,
            label: col.name,
          })),
        ],
      },
      {
        ...
        columns: PAYMENT_COLS.filter((col) => col.key !== "name").map(
          (col) => ({ key: `payment:${col.key}`, label: col.label }),
        ),
      },
    ],
  ...
  useEffect(() => {
    try {
      ...
      const clientOrderOwner = localStorage.getItem(
        "colOrder:clients:local_owner",
      );
      const subitemOrderOwner = localStorage.getItem(
        "colOrder:subitems:local_owner",
      );
      const paymentOrderOwner = localStorage.getItem(
        "colOrder:payments:local_owner",
      );
      ...
      if (currentUserId) {
        ...
        if (clientOwner && clientOwner !== currentUserId) {
          try {
            localStorage.removeItem("colWidths:clients:local");
            localStorage.removeItem("colWidths:clients:local_owner");
          } catch {}
        ...
        if (subOwner && subOwner !== currentUserId) {
          try {
            localStorage.removeItem("colWidths:subitems:local");
            localStorage.removeItem("colWidths:subitems:local_owner");
          } catch {}
          // notify subitems to reset to defaults
          try {
            window.dispatchEvent(
              new CustomEvent("subitemColsChanged", {
        ...
        if (payOwner && payOwner !== currentUserId) {
          try {
            localStorage.removeItem("colWidths:payments:local");
            localStorage.removeItem("colWidths:payments:local_owner");
          } catch {}
          try {
            window.dispatchEvent(
              new CustomEvent("paymentColsChanged", {
                detail: Object.fromEntries(
                  PAYMENT_COLS.map((c) => [c.key, c.width]),
                ),
              }),
        ...
        if (clientOrderOwner && clientOrderOwner !== currentUserId) {
          try {
            localStorage.removeItem("colOrder:clients:local");
            localStorage.removeItem("colOrder:clients:local_owner");
          } catch {}
          setHeaderCols(CLIENT_HEADER_COLS.map((c) => ({ ...c })));
        }
        if (subitemOrderOwner && subitemOrderOwner !== currentUserId) {
          try {
            localStorage.removeItem("colOrder:subitems:local");
            localStorage.removeItem("colOrder:subitems:local_owner");
          } catch {}
          try {
            window.dispatchEvent(
              new CustomEvent("subitemColsReordered", {
                detail: SUBITEM_COLS.map((c) => c.key),
              }),
            );
          } catch {}
        }
        if (paymentOrderOwner && paymentOrderOwner !== currentUserId) {
          try {
            localStorage.removeItem("colOrder:payments:local");
            localStorage.removeItem("colOrder:payments:local_owner");
          } catch {}
          try {
            window.dispatchEvent(
              new CustomEvent("paymentColsReordered", {
                detail: PAYMENT_COLS.map((c) => c.key),
              }),
            );
          } catch {}
        }
    ...
    try {
      window.dispatchEvent(
        new CustomEvent("authChanged", { detail: currentUserId }),
      );
    } catch {}
  }, [currentUserId]);
  ...
  useEffect(() => {
    try {
      ...
      setHeaderCols((prev) => {
        const fixedFront = prev.filter((c) =>
          ["selectCheckbox", "client"].includes(c.key),
        );
        const fixedEnd = prev.filter((c) =>
          ["addClientCol", "empty"].includes(c.key),
        );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setHeaderCols((prev) => {
          const fixedFront = prev.filter((col) =>
            ["selectCheckbox", "client"].includes(col.key),
          );
          const fixedEnd = prev.filter((col) =>
            ["addClientCol", "empty"].includes(col.key),
          );
          const middle = prev.filter(
            (col) =>
      ...
      setCustomClientWidths(
        Object.fromEntries(
          Object.entries(map).filter(
            ([key, value]) =>
              key.startsWith("custom:") && typeof value === "number",
          ),
        ),
      );
      setHeaderCols((prev) =>
        prev.map((c) => ({
          ...c,
          width: c.key === "empty" ? 44 : (map[c.key] ?? c.width),
        })),
      );
    } catch (e) {
      // ignore
      ...
      try {
        ...
        if (value && typeof value === "object") {
          setCustomClientWidths(
            Object.fromEntries(
              Object.entries(value as Record<string, unknown>).filter(
                ([key, width]) =>
                  key.startsWith("custom:") && typeof width === "number",
              ),
          ...
          setHeaderCols((prev) =>
            prev.map((c) => ({
              ...c,
              width: c.key === "empty" ? 44 : (value[c.key] ?? c.width),
            })),
          );
          return;
        }
    ...
    (async () => {
      try {
        ...
        const map = Object.fromEntries(
          mergedHeaderCols.map((c) => [c.key, c.width]),
        );
        ...
        try {
          localStorage.setItem("colWidths:clients:local", JSON.stringify(map));
        } catch {}
        try {
          localStorage.setItem(
            "colWidths:clients:local_owner",
  ...
  const updateClientCustomField = useCallback(
    async (clientId: string, columnId: string, value: string) => {
      ...
      setClients((prev) =>
        prev.map((c) =>
          c.id === clientId ? { ...c, customFields: nextCustomFields } : c,
        ),
      );
    ...
    const loadAssignments = async () => {
      try {
        ...
        const navigationGroupId = searchTarget
          ? clients.find((client) => client.id === searchTarget.clientId)
              ?.groupId
          : null;
        setCollapsedGroups(
          Object.fromEntries(
  ...
  const renameOptionValue = useCallback(
    async (code: string, oldName: string, newName: string) => {
      ...
      setters[code]?.((previous) =>
        previous.map((entry) =>
          entry.value === oldName ? { ...entry, value: trimmed } : entry,
        ),
      );
      await reloadClients();
  ...
  const handleAddReplyStatus = useCallback(
    async (name: string) => {
      const trimmed = name.trim();
      if (!trimmed) return;
      const supabase = createSupabaseClient();
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          value: trimmed,
          color: "#d1d5db",
          sort_order: replyStatuses.length,
        })
        .select("value, color")
        .single();
      if (error) {
        console.error(error);
        toast.error("Reply status could not be added", {
          description: error.message,
        });
        return;
      }
  ...
  const handleAddStatus = useCallback(
    async (name: string) => {
      ...
      if (error) {
        console.error(error);
        toast.error("Status could not be added", {
          description: error.message,
        });
        return;
      }
    ...
    async (name: string) => {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          value: trimmed,
          color: "#d1d5db",
          sort_order: channelOptions.length,
        })
        .select("value, color")
        .single();
      if (error) {
        console.error(error);
        toast.error("Channel could not be added", {
          description: error.message,
        });
        return;
      }
    ...
    async (name: string) => {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          value: trimmed,
          color: "#d1d5db",
          sort_order: importanceOptions.length,
        })
        .select("value, color")
        .single();
      if (error) {
        console.error(error);
        toast.error("Importance could not be added", {
          description: error.message,
        });
        return;
      }
  ...
  const handleAddGroup = useCallback(
    async (name: string) => {
      if (!["director", "admin", "dev"].includes(currentUserRole ?? "")) {
        toast.error("Group creation is restricted", {
          description:
            "Only directors, admins, and dev users can create groups.",
        });
        return;
      }
      const trimmed = name.trim();
      ...
      try {
        ...
        notifyChange(
          "Group added",
          `${trimmed} is now available on the board.`,
        );
      } catch (error) {
        console.error("Failed to add group", error);
        toast.error("Group could not be added", {
          description:
            error instanceof Error ? error.message : "The group was not saved.",
        });
      }
    },
  ...
  const handleDeleteGroup = useCallback(async () => {
    ...
    try {
      ...
      const clientIdsInGroup = clients
        .filter((c) => c.groupId === groupToDelete.id)
        .map((c) => c.id);
      ...
      notifyChange(
        "Group deleted",
        `${groupToDelete.name} and its clients were removed.`,
      );
      setGroupToDelete(null);
    } catch (error) {
      console.error("Failed to delete group", error);
      toast.error("Group could not be deleted", {
        description:
          error instanceof Error ? error.message : "The group was not deleted.",
      });
    } finally {
  ...
  const confirmDeleteCustomColumn = useCallback(async () => {
    ...
    try {
      ...
      setCustomColumns((prev) =>
        prev.filter((c) => c.id !== pendingDeleteCustomColumn.id),
      );
      ...
      toast.error("Column could not be deleted", {
        description:
          e instanceof Error ? e.message : "The custom column was not deleted.",
      });
    } finally {
  ...
  const handleDragStart = useCallback(
    (clientId: string, event: React.DragEvent) => {
  ...
  const handleDragOver = useCallback(
    (e: React.DragEvent, groupId: string, edge: "top" | "bottom") => {
      if (
  ...
  const handleSubitemDragOver = useCallback(
    (event: React.DragEvent<HTMLDivElement>, clientId: string) => {
      if (
        !Array.from(event.dataTransfer.types).includes(
          "application/x-crm-subitem",
        )
      )
        return;
  ...
  const handleSubitemRowDragOver = useCallback(
    (
  ...
  const handleSubitemRowDrop = useCallback(
    async (
      event: React.DragEvent<HTMLTableRowElement>,
      clientId: string,
      targetSubitemId: string,
      ...
      event.stopPropagation();
      const marker =
        subitemDropMarker?.clientId === clientId &&
        subitemDropMarker.subitemId === targetSubitemId
          ? subitemDropMarker
      ...
      const moving = client.subitems.find(
        (item) => item.id === draggedSubitem.id,
      );
      if (!moving) {
        handleSubitemDragEnd();
        return;
      ...
      setClients((previous) =>
        previous.map((item) =>
          item.id === clientId ? { ...item, subitems: positioned } : item,
        ),
      );
      try {
        await reorderSubitemRows(
          clientId,
          positioned.map((item) => item.id),
        );
      } catch (error) {
        console.error("Failed to reorder subitems", error);
        toast.error("Subitems could not be reordered", {
          description:
  ...
  const handleSubitemDrop = useCallback(
    async (event: React.DragEvent<HTMLDivElement>, targetClientId: string) => {
      if (
        !Array.from(event.dataTransfer.types).includes(
          "application/x-crm-subitem",
        )
      ...
      handleSubitemDragEnd();
      if (!movingSubitem || movingSubitem.sourceClientId === targetClientId)
        return;

      const sourceClient = clients.find(
        (client) => client.id === movingSubitem.sourceClientId,
      );
      const subitem = sourceClient?.subitems.find(
        (item) => item.id === movingSubitem.id,
      );
      if (!subitem) return;
      ...
      setClients((previous) =>
        previous.map((client) => {
          if (client.id === movingSubitem.sourceClientId) {
            return {
              ...client,
              subitems: client.subitems.filter(
                (item) => item.id !== movingSubitem.id,
              ),
            };
          }
      ...
      try {
        ...
        await reloadClients();
        toast.error("Subitem could not be moved", {
          description:
            error instanceof Error
              ? error.message
              : "The subitem was not moved.",
        });
      }
    },
  ...
  const handleGroupDragStart = useCallback(
    (groupId: string, event: React.DragEvent) => {
  ...
  const handleGroupDrop = useCallback(
    async (targetGroupId: string, edge: "top" | "bottom") => {
  ...
  const advancedColumns = useMemo<AdvancedFilterColumn[]>(() => {
    ...
    const clientColumns = mergedHeaderCols
      .filter(
        (column) =>
          !["selectCheckbox", "addClientCol", "empty"].includes(column.key),
      )
      .map((column) => ({
    ...
    const subitemColumns = allSubitemColumns.map((column) => ({
      key: `subitem:${column.key}`,
      label: column.label,
      category: "Subitem" as const,
      values: unique(
  ...
  const matchesAdvancedRule = useCallback(
    (client: Client, rule: AdvancedFilterRule) => {
      ...
      if (rule.condition === "is not" || rule.condition === "text is not")
        return tests.every((value) => value !== query);
      if (rule.condition === "contains")
        return tests.some((value) => value.includes(query));
      if (rule.condition === "does not contain")
        return tests.every((value) => !value.includes(query));
  ...
  const displayedClients = clients.filter((client) => {
    ...
    const populatedRules = advancedRules.filter(
  ...
  const clientSortColumns = mergedHeaderCols.filter(
    (column) =>
      !["selectCheckbox", "addClientCol", "empty"].includes(column.key),
  );
  const subitemSortColumns = [
    ...SUBITEM_COLS,
    ...subitemCustomCols.map((column) => ({
      key: `custom:${column.id}`,
      label: column.name,
  ...
  const selectedSortColumns =
    boardSort.category === "client"
      ? clientSortColumns
      : boardSort.category === "subitem"
        ? subitemSortColumns
        : paymentSortColumns;
  const activeBoardSort = selectedSortColumns.some(
    (column) => column.key === boardSort.column,
  )
    ? boardSort
    : DEFAULT_BOARD_SORT;
  const clientSortValue = (client: Client, column: string): string | number => {
    ...
    if (column === "people")
      return (clientAssignees[client.id] ?? [])
        .map(
          (id) =>
            peopleProfilesById[id]?.full_name ||
            peopleProfilesById[id]?.email ||
        ...
        .join(", ");
    if (column === "pm")
      return clientPmAssigneeIds(client)
        .map(
          (id) =>
            peopleProfilesById[id]?.full_name ||
            peopleProfilesById[id]?.email ||
  ...
  const compareClients = (first: Client, second: Client) => {
    ...
    const firstBlank =
      firstValue === "" || firstValue === null || firstValue === undefined;
    const secondBlank =
      secondValue === "" || secondValue === null || secondValue === undefined;
    if (firstBlank !== secondBlank) return firstBlank ? 1 : -1;

    const columnDefinition = clientSortColumns.find(
      (item) => item.key === column,
    );
    const isDate =
      ["followUp", "nbd", "dateCreated"].includes(column) ||
      columnDefinition?.field_type === "date";
    const isNumber =
      ["totalPrice", "totalMarkup"].includes(column) ||
      columnDefinition?.field_type === "number";
    let comparison = 0;
    if (isDate) {
      ...
      const firstNumber =
        Number(String(firstValue).replace(/[^0-9.-]/g, "")) || 0;
      const secondNumber =
        Number(String(secondValue).replace(/[^0-9.-]/g, "")) || 0;
    ...
    if (comparison !== 0)
      return activeBoardSort.direction === "asc" ? comparison : -comparison;
    return (
      new Date(second.createdAt || 0).getTime() -
      new Date(first.createdAt || 0).getTime()
    );
  ...
  const subitemSortValue = (
    ...
    if (column === "people")
      return (subitemAssignees[subitem.id] ?? [])
        ...
        .join(", ");
    if (column.startsWith("custom:"))
      return subitem.customFields?.[column.slice(7)] ?? "";
    if (
      column === "cSgd" ||
      column === "tcSgd" ||
      column === "tc" ||
      column === "price" ||
      column === "markup" ||
      column === "percentMarkup"
    )
      return financials[column] ?? "";
    if (column === "priceToSet") {
      const idealMarkup = Number(subitem.customFields?.idealMarkup || 0);
      return financials.quantity > 0
        ? (idealMarkup + financials.tc) / financials.quantity
        : "";
    }
    if (column === "idealMarkup")
      return subitem.customFields?.idealMarkup ?? "";
    if (column === "totalUc")
  ...
  const compareSubitems = (first: Subitem, second: Subitem) => {
    ...
    const fieldType =
      definition && "field_type" in definition
    ...
    const isDate =
      activeBoardSort.column === "createdAt" || fieldType === "date";
    const isNumber =
      numericKeys.has(activeBoardSort.column) || fieldType === "number";
    let comparison = 0;
    if (isDate)
      comparison =
  ...
  const groupedClients = groups.map((group) => ({
    ...
    clients: displayedClients
      ...
      .sort(compareClients)
      .map((client) =>
        activeBoardSort.category === "client"
          ? client
  ...
  const selectedSubitems = useMemo(
    () =>
      clients.flatMap((client) =>
        client.subitems.filter((subitem) =>
  ...
  const openCombinedPush = async () => {
    ...
    const owner = clients.find((client) =>
      client.subitems.some((subitem) => subitem.id === selectedSubitemIds[0]),
    );
    const orderedSelectedIds =
      owner?.subitems
        .filter((subitem) => selectedSubitemIds.includes(subitem.id))
        .map((subitem) => subitem.id) ?? selectedSubitemIds;
    const owners = new Set(
      orderedSelectedIds.map(
        (id) =>
          clients.find((client) =>
            client.subitems.some((subitem) => subitem.id === id),
          )?.id,
      ),
    );
    if (owners.size !== 1)
      return toast.error(
        "Combined shipments can only include subitems from the same client.",
      );
    ...
    try {
      ...
      const preview: CombinedPushPreview = {
        shipperName: String(first.shipper_name || "selected shipper"),
        page: 0,
        existingMode: "separate",
        rows: rows.map((row: any) => ({
          ...
          sea_or_air: String(row.sea_or_air || ""),
        })),
        shared: {
          info_provided_date: String(first.info_provided_date || today),
          tax_refund:
            rows.reduce(
              (sum: number, row: any) =>
                sum + (Number(row.qty) || 0) * (Number(row.up) || 0),
              0,
            ) > 2500
              ? "退"
              : "X",
          cn_tracking_no: "",
          cartons: String(first.cartons ?? ""),
          delivery_info: String(first.delivery_info || ""),
          sea_or_air: String(first.sea_or_air || ""),
          samples_by_air: String(first.samples_by_air ?? ""),
          samples_by_sea: String(first.samples_by_sea ?? ""),
          serial_number: "",
          waybill_date: "",
          waybill_number: "",
          pieces: "",
  ...
  const confirmCombinedPush = async () => {
    ...
    setLoadingCombinedPush(true);
    try {
      const response = await fetch("/api/shipper/combined-push", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          subitemIds: combinedPushPreview.rows.map((row) => row.subitemId),
          values: combinedPushPreview.rows,
          shared: combinedPushPreview.shared,
          amendShipmentIdBySubitemId:
      ...
      setClients((current) =>
        current.map((client) => ({
          ...client,
          subitems: client.subitems.map((subitem) => {
            const value = combinedPushPreview.rows.find(
              (row) => row.subitemId === subitem.id,
            );
            return value
              ? { ...subitem, cnTracking: value.cn_tracking_no }
              : subitem;
          }),
        })),
      );
            ...
            const clientTotals = client.subitems.reduce(
              (subitemTotals, subitem) => {
                ...
                return {
                  ...
                  totalUp:
                    subitemTotals.totalUp + parseSubitemNumber(subitem.up),
  ...
  const orderedMoveGroups = useMemo(() => {
    const orderedGroups = groups
      .map((group) => ({
        name: group.name,
        clients: parentClientOptions.filter(
          (client) =>
    ...
    const ungroupedClients = parentClientOptions.filter(
      (client) =>
    ...
    return ungroupedClients.length
      ? [...orderedGroups, { name: "Ungrouped", clients: ungroupedClients }]
      : orderedGroups;
    ...
    >
      ...
      <div className="max-h-60 space-y-1 overflow-y-auto pr-1">
        <button
          ...
          className="flex items-center gap-2 w-full rounded-md px-2 py-1.5 text-left text-[10px] font-semibold hover:bg-gray-50"
        >
        ...
        {options.map((option) => {
          ...
          return (
            ...
            >
              {renderOption ? (
                renderOption(optionValue, optionLabel)
              ) : (
                <span
                  className="h-2.5 w-2.5 rounded-sm bg-[#7BCBD5]"
                  style={
                    colors?.[optionValue]
                      ? { background: colors[optionValue] }
                      : undefined
                  }
                />
              )}
  ...
  const toggleCollapseAllGroups = useCallback(() => {
    ...
    const groupIds = groups
      .map((group) => group.id)
      .filter((id) => !loadedGroupContentIds.has(id));
    if (shouldExpand) {
      setPendingGroupContentIds(new Set(groupIds));
      ...
      setPendingGroupContentIds(new Set());
    }
    setCollapsedGroups(
      Object.fromEntries(groups.map((group) => [group.id, !shouldExpand])),
    );
  }, [collapsedGroups, groups, loadedGroupContentIds]);

  const toggleGroup = useCallback(
    (id: string) => {
      const isExpanding = Boolean(collapsedGroups[id]);
      if (isExpanding && !loadedGroupContentIds.has(id)) {
        setPendingGroupContentIds((current) => new Set(current).add(id));
        window.requestAnimationFrame(() => {
          window.requestAnimationFrame(() => {
            setLoadedGroupContentIds((current) => new Set(current).add(id));
            setPendingGroupContentIds((current) => {
              const next = new Set(current);
              next.delete(id);
              return next;
    ...
    [collapsedGroups, loadedGroupContentIds],
  );
  ...
  const toggleSelectGroup = useCallback(
    (groupClientIds: string[]) => {
      ...
      setSelectedIds((previous) => {
        const next = new Set(previous);
        const allGroupClientsSelected = groupClientIds.every((clientId) =>
          next.has(clientId),
        );
        groupClientIds.forEach((clientId) =>
          allGroupClientsSelected ? next.delete(clientId) : next.add(clientId),
        );
        return next;
      });
    },
  ...
  const handleSubitemAssigneesChange = useCallback(
    async (subitemId: string, ids: string[]) => {
      ...
      try {
        ...
        const subitem = parentClient?.subitems.find(
  ...
  const updateClient = useCallback(
    async (
      ...
      const customerField =
        updates.phone !== undefined && updates.phone !== existingClient?.phone
          ? "phone"
          : updates.company !== undefined &&
              updates.company !== existingClient?.company
            ? "company"
            : null;
      if (existingClient && customerField) {
        ...
        if (!value) {
          toast.error(
            ...
            {
              description:
                "A customer profile can only be matched using a filled value.",
            },
          );
          return;
        }
      ...
      const selectedGroup = updates.groupId
        ? groups.find((group) => group.id === updates.groupId)
        : null;
      const isMovingToClosedLeads = !!selectedGroup?.name
        .trim()
        .toLowerCase()
      ...
      if (updates.status === "Closed") {
        try {
          ...
          toast.error("Closed Leads group could not be prepared", {
            description:
              error instanceof Error
                ? error.message
                : "The lead was not closed.",
          });
          return;
        }
      ...
      try {
        ...
        if (movedToGroupName && nextUpdates.status) {
          ...
          const statusToastId = toast.success(
            `${clientName} moved to ${movedToGroupName}`,
            {
              ...
              action: {
                ...
                onClick: () => {
                  ...
                  const rollback = {
                    ...
                    groupId: previousClient.groupId,
                  } as Partial<Client>;
                  void updateClient(clientId, rollback)
                    .then(() => {
                      toast.dismiss(statusToastId);
                      toast.success("Automation undone", {
                        description: `${clientName} was restored to its previous status and group.`,
                      });
                    })
                    .catch((error) =>
                      toast.error("Undo failed", {
                        description:
                          error?.message ||
                          "The previous status could not be restored.",
                      }),
                    );
                },
              },
  ...
  const confirmCloseLead = useCallback(async () => {
    ...
    try {
      ...
      try {
        const parsed = JSON.parse(client.customFields?.closedLeadFiles ?? "[]");
        if (Array.isArray(parsed)) existingFiles = parsed;
      } catch {}
      await updateClient(
        pendingCloseLead.clientId,
        {
          ...
          customFields: {
            ...
            ...(signedOcfCheck.signedAt
              ? { closedLeadOcfSignedAt: signedOcfCheck.signedAt }
              : {}),
          },
        },
        true,
  ...
  const undoActivity = useCallback(
    async (entry: import("../app/types").ActivityEntry) => {
      ...
      if (entry.action === "field_changed") {
        ...
        if (!canEditClientRecord(entry.clientId)) {
          showAssignmentPermissionError();
          return;
        }
      ...
      if (
        ...
        entry.subitemId &&
        entry.fieldName &&
        !entry.fieldName.startsWith("timeline:")
      ) {
        if (
          !entry.clientId ||
          !canEditSubitemRecord(entry.clientId, entry.subitemId)
        ) {
          showAssignmentPermissionError();
          return;
        }
        if (entry.fieldName === "parentClient") {
          const oldClientId =
            typeof entry.meta?.oldClientId === "string"
              ? entry.meta.oldClientId
              : null;
          const newClientId =
            typeof entry.meta?.newClientId === "string"
              ? entry.meta.newClientId
              : null;
            ...
            previous.map((client) => {
              if (client.id === newClientId) {
                return {
                  ...client,
                  subitems: client.subitems.filter(
                    (subitem) => subitem.id !== entry.subitemId,
                  ),
                };
              }
              if (client.id === oldClientId) {
                const movedSubitem = clients
                  .find((item) => item.id === newClientId)
                  ?.subitems.find((subitem) => subitem.id === entry.subitemId);
                return movedSubitem
                  ? { ...client, subitems: [...client.subitems, movedSubitem] }
                  : client;
              }
              return client;
            }),
          ...
          toast.success("Move undone", {
            description: "The subitem was moved back to its previous client.",
          });
          return;
        }
  ...
  const pendingSubitemToDelete = useMemo(() => {
    ...
    const client = clients.find(
      (item) => item.id === pendingDeleteSubitem.clientId,
    );
    const subitem = client?.subitems.find(
      (item) => item.id === pendingDeleteSubitem.subitemId,
    );
    return {
  ...
  const deleteSelected = useCallback(async () => {
    ...
    if (ids.some((clientId) => !canEditClientRecord(clientId))) {
      showAssignmentPermissionError();
      return;
    }
    ...
    try {
      ...
      notifyChange(
        "Clients deleted",
        `${ids.length} selected client${ids.length === 1 ? "" : "s"} were removed.`,
      );
    } catch (error: any) {
      setClients(clients);
  ...
  const moveSelectedClients = useCallback(
    async (targetGroupId: string) => {
      ...
      try {
        ...
        if (updates.status === "Closed") {
          if (selectedIds.size !== 1) {
            toast.error("Close leads one at a time", {
              description:
                "Each lead requires its own purchase order, signed quotation, proof of payment, and OCF confirmation.",
            });
            return;
          }
        ...
        await reloadClients();
        toast.success("Clients moved", {
          description: `${selectedIds.size} selected client${selectedIds.size === 1 ? "" : "s"} were moved to the chosen group.`,
        });
        setSelectedIds(new Set());
      } catch (error) {
        await reloadClients();
        toast.error("Clients could not be moved", {
          description:
            error instanceof Error
              ? error.message
              : "The selected clients were not moved.",
        });
      } finally {
    ...
    async (clientId: string, name: string) => {
      ...
      try {
        ...
        setClients((previous) =>
          previous.map((client) =>
            client.id === clientId
              ? { ...client, subitems: [...client.subitems, createdSubitem] }
              : client,
          ),
        );
        if (currentUserId)
          setSubitemAssignees((previous) => ({
            ...previous,
            [createdSubitem.id]: [currentUserId],
          }));
        notifyChange(
          "Subitem added",
        ...
        toast.error("Subitem could not be added", {
          description: error?.message || "The subitem was not saved.",
        });
        throw error;
      }
    },
      ...
      try {
        await Promise.all(
          subitemIds.map((subitemId) =>
            moveSubitemRow(subitemId, targetClientId),
          ),
        );
        await reloadClients();
        toast.success("Subitems moved", {
          description: `${subitemIds.length} subitem${subitemIds.length === 1 ? "" : "s"} moved to the selected client.`,
        });
      } catch (error) {
        await reloadClients();
        toast.error("Subitems could not be moved", {
          description:
            error instanceof Error
              ? error.message
              : "The subitems were not moved.",
        });
      } finally {
  ...
  const duplicateSelectedSubitems = useCallback(async () => {
    ...
    try {
      ...
      await reloadClients();
      toast.error("Subitems could not be duplicated", {
        description:
          error instanceof Error
            ? error.message
            : "The selected subitems were not duplicated.",
      });
    } finally {
  ...
  const duplicateSubitemAction = useCallback(
    async (subitemId: string) => {
      const owner = clients.find((client) =>
        client.subitems.some((subitem) => subitem.id === subitemId),
      );
      if (!owner || !canEditSubitemRecord(owner.id, subitemId)) {
        showAssignmentPermissionError();
        return;
      }
      try {
  ...
  const moveSubitemAction = useCallback(
    async (subitemId: string, targetClientId: string) => {
  ...
  const toggleAllSubitems = useCallback(
    (subitemIds: string[]) => {
      if (selectedIds.size > 0) return;
      setSelectedSubitemIds((previous) => {
        const allSelected =
          subitemIds.length > 0 &&
          subitemIds.every((id) => previous.includes(id));
        return allSelected
  ...
  return (
    <div className="crm-board flex flex-col h-full bg-white">
      {detailSubitem &&
        (() => {
          const owner = clients.find(
            (client) => client.id === detailSubitem.clientId,
          );
          const subitem = owner?.subitems.find(
            (item) => item.id === detailSubitem.subitemId,
          );
          ...
          return (
            <SubitemDetailView
              ...
              onUpdate={(updates) =>
                updateSubitem(owner.id, subitem.id, updates)
              }
              ...
              options={{
                ...
                localOverseas: localOverseasEntries,
                shipper: shipperEntries,
                currency: currencyEntries,
                payment: paymentEntries,
                paymentStatus: paymentStatusEntries,
                modeOfPayment: modeOfPaymentEntries,
                subProgress: subitemSubprogressEntries,
              }}
              moveTargetGroups={groupedClients.map(
              ...
              onDuplicate={() => duplicateSubitemAction(subitem.id)}
              onMove={(targetClientId) =>
                moveSubitemAction(subitem.id, targetClientId)
              }
              onDelete={() => {
                setDetailSubitem(null);
      ...
      {detailClientId &&
        (() => {
          const detailClient = clients.find(
            (client) => client.id === detailClientId,
          );
          ...
          return (
            <ClientDetailView
              key={detailClient.id}
              client={detailClient}
              clients={groupedClients.flatMap(
                ({ clients: groupClients }) => groupClients,
              )}
              profiles={profiles}
              assigneeIds={clientAssignees[detailClient.id] ?? []}
              pmIds={clientPmAssigneeIds(detailClient)}
              canEdit={canEditClientRecord(detailClient.id)}
      ...
      {selectedIds.size > 0 && (
        <div className="fixed bottom-8 left-1/2 z-[100] flex min-h-16 w-[min(1100px,calc(100vw-2rem))] -translate-x-1/2 items-center gap-2 overflow-x-auto rounded-2xl border border-slate-200 bg-white px-4 py-4 shadow-2xl">
          ...
          >
            <Copy size={17} />{" "}
            {isDuplicatingClients ? "Duplicating..." : "Duplicate"}
          </button>
          ...
          <button
            type="button"
            disabled={!canEditSelectedClients}
      ...
      {selectedSubitemIds.length > 0 && (
        <div className="fixed bottom-8 left-1/2 z-[100] flex min-h-16 w-[calc(100vw-3.5rem)] max-w-[1100px] -translate-x-1/2 items-center gap-2 overflow-x-auto rounded-2xl border border-slate-200 bg-white px-4 py-4 shadow-2xl">
          ...
          <button
            ...
            onClick={() =>
              selectedSubitemIds.length === 1
                ? window.dispatchEvent(
                    new CustomEvent("openShipperPush", {
                      detail: { subitemId: selectedSubitemIds[0] },
                    }),
                  )
                : void openCombinedPush()
            }
            title={
              !canAccessShipperPush
          ...
          >
            <Copy size={17} />{" "}
            {isDuplicatingSubitems ? "Duplicating..." : "Duplicate"}
          </button>
          <div className="relative">
            <button
              type="button"
      ...
      <div className="flex items-center gap-2 px-2 py-1 border-b border-gray-200 bg-white flex-shrink-0">
        ...
        <div ref={filterRef} className="relative">
          ...
          {showFilter && (
            <div
              className={`absolute top-full left-0 z-50 mt-1 rounded-lg ${filterMode === "quick" ? "w-[min(680px,calc(100vw-1rem))] border border-gray-200 bg-white p-3 shadow-xl" : ""}`}
            >
              {filterMode === "advanced" && (
                <AdvancedFilters
              ...
              <div
                className={`${filterMode === "advanced" ? "hidden" : ""} mb-3 flex items-center justify-between border-b border-gray-100 pb-2`}
              >
                <div className="flex items-center gap-2">
                  {focusedFilterColumn && (
                    <button
                      onClick={() => setFocusedFilterColumn(null)}
                  ...
                  <span className="text-xs font-semibold text-gray-800">
                    Quick filters
                    {focusedFilterColumn ? ` · ${focusedFilterColumn}` : ""}
                  </span>
                </div>
                <button
                  onClick={() => {
              ...
              <div
                className={`${filterMode === "advanced" ? "hidden" : ""} flex gap-3 overflow-x-auto pb-1`}
              >
                {renderFilterColumn({
                  label: "Status",
                  value: filterStatus,
                ...
                {renderFilterColumn({
                  ...
                  renderOption: (value, label) => {
                    ...
                    const displayLabel =
                      profile?.full_name || profile?.email || label;
                    const initials =
                      displayLabel
                        .trim()
                        .split(/\s+/)
              ...
              <div
                className={`${filterMode === "advanced" ? "absolute bottom-4 right-5" : "mt-3 border-t border-gray-100 pt-2 text-right"}`}
              >
                <button
                  type="button"
                  onClick={() =>
                    setFilterMode((mode) =>
                      mode === "advanced" ? "quick" : "advanced",
                    )
                  }
        ...
        <div className="relative" data-crm-menu-trigger>
          ...
          {showSortMenu && (
            <div
              data-crm-menu
              className="absolute left-0 top-full z-50 mt-1 w-[430px] max-w-[calc(100vw-1rem)] rounded-xl border border-slate-200 bg-white p-4 shadow-xl"
            >
              <div className="mb-3">
                ...
                <p className="mt-0.5 text-xs text-slate-500">
                  Client sorting applies within groups; subitem and payment
                  sorting applies within each client.
                </p>
              </div>
              <div className="grid gap-2 sm:grid-cols-[1fr_150px]">
                <label className="grid gap-1 text-[11px] font-medium text-slate-500">
                  Column
                  <select
                    value={`${activeBoardSort.category}:${activeBoardSort.column}`}
                    onChange={(event) => {
                      const [category, ...columnParts] =
                        event.target.value.split(":");
                      setBoardSort((current) => ({
                        ...current,
                        category: category as BoardSortSetting["category"],
                        column: columnParts.join(":"),
                      }));
                    }}
                    className="h-10 rounded-md border border-slate-300 bg-white px-3 text-sm text-slate-700 outline-none focus:border-sky-400"
                  >
                    <optgroup label="Client columns">
                      {clientSortColumns.map((column) => (
                        <option
                          key={`client:${column.key}`}
                    ...
                    <optgroup label="Subitem columns">
                      {subitemSortColumns.map((column) => (
                        <option
                          key={`subitem:${column.key}`}
                          value={`subitem:${column.key}`}
                        >
                          {column.label || column.key}
                        </option>
                      ))}
                    </optgroup>
                    <optgroup label="Payment columns">
                      {paymentSortColumns.map((column) => (
                        <option
                          key={`payment:${column.key}`}
                          value={`payment:${column.key}`}
                        >
                          {column.label || column.key}
                        </option>
                      ))}
                ...
                <label className="grid gap-1 text-[11px] font-medium text-slate-500">
                  Direction
                  <select
                    value={activeBoardSort.direction}
                    onChange={(event) =>
                      setBoardSort((current) => ({
                        ...current,
        ...
        <div className="relative" data-crm-menu-trigger>
          ...
          {showHideColumns && (
            <div
              data-crm-menu
              className="absolute top-full left-0 mt-1 w-64 max-h-[min(520px,calc(100vh-5rem))] overflow-y-auto rounded-lg border border-gray-200 bg-white p-2 shadow-xl z-50"
            >
              <div className="flex items-center justify-between border-b border-gray-100 px-2 pb-2">
                ...
                <button
                  onClick={() => setHiddenColumnKeys(new Set())}
                ...
                <div key={group.label} className="pt-2">
                  ...
                  {group.columns.map((column) => {
                    ...
                    return (
                      <label
                        key={column.key}
                        className="flex cursor-pointer items-center gap-2 rounded-md px-2 py-1.5 text-[11px] text-gray-700 hover:bg-gray-50"
                      >
                        <input
                          type="checkbox"
                          checked={visible}
                          ...
                          className="h-3.5 w-3.5 rounded accent-[#0f8da8]"
                        />
                        <span className="truncate">
                          {column.label || "Unnamed column"}
                        </span>
                      </label>
                    );
                  })}
                ...
                customerMatchPending.exactProfile.id !==
                  ...
                  <div className="rounded-lg border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900">
                    <p className="font-semibold">
                      This value already belongs to another profile
                    </p>
                    <p className="mt-1 text-xs">
                      ...
                      <span className="font-semibold">
                        {customerMatchPending.exactProfile.name}
                      </span>
                      . Choosing “different customer” will detach this lead from
                      its original profile and link it to that existing profile.
              ...
              {customerMatchPending.exactProfile &&
                customerMatchPending.exactProfile.id ===
                  ...
                  <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-xs text-emerald-800">
                    This value already belongs to the customer profile currently
                    linked to this lead.
                  </div>
                )}
              {customerMatchPending.field === "company" &&
                customerMatchPending.suggestions.length > 0 && (
                  <div className="rounded-lg border border-violet-200 bg-violet-50/50 p-3">
                    <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-violet-700">
                      Similar company profiles
                    </p>
                    <div className="space-y-1.5">
                      {customerMatchPending.suggestions.map((suggestion) => (
                        <button
                          key={suggestion.id}
                          type="button"
      ...
      <AlertDialog
        ...
        onOpenChange={setPendingDeleteSelected}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
      ...
      <AlertDialog
        open={!!pendingDeleteClientId}
        onOpenChange={(open) => !open && setPendingDeleteClientId(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
      ...
      <AlertDialog
        open={!!pendingDeleteSelectedSubitems}
        onOpenChange={(open) => !open && setPendingDeleteSelectedSubitems(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Choose which section&apos;s widths to restore. Subitem also
              includes payment columns.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction
              onClick={async () => {
                ...
                await handleRestoreDefaults("client");
              }}
            >
              Client only
            </AlertDialogAction>
            <AlertDialogAction
              onClick={async () => {
                setShowRestoreConfirm(false);
                await handleRestoreDefaults("subitem");
              }}
            >
              Subitem only
            </AlertDialogAction>
            <AlertDialogAction
              onClick={async () => {
                setShowRestoreConfirm(false);
                await handleRestoreDefaults("all");
              }}
            >
              Restore all
            </AlertDialogAction>
      ...
      <AlertDialog
        open={showRestoreArrangementConfirm}
        onOpenChange={setShowRestoreArrangementConfirm}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
            ...
            <AlertDialogDescription>
              Choose which section&apos;s arrangement to restore. Subitem also
              includes payment columns.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={async () => {
                setShowRestoreArrangementConfirm(false);
                await handleRestoreDefaultArrangement("client");
              }}
            >
              Client only
            </AlertDialogAction>
            <AlertDialogAction
              onClick={async () => {
                setShowRestoreArrangementConfirm(false);
                await handleRestoreDefaultArrangement("subitem");
              }}
            >
              Subitem only
            </AlertDialogAction>
            <AlertDialogAction
              onClick={async () => {
                setShowRestoreArrangementConfirm(false);
                await handleRestoreDefaultArrangement("all");
              }}
            >
              Restore all
            </AlertDialogAction>
      ...
      <AlertDialog
        open={showRestoreSortingConfirm}
        onOpenChange={setShowRestoreSortingConfirm}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Restore default column sorting?</AlertDialogTitle>
            <AlertDialogDescription>
              Choose the section to restore. The board default is clients by
              Date Created, newest first; subitem and payment rows return to
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => {
                setShowRestoreSortingConfirm(false);
                if (boardSort.category === "client")
                  setBoardSort(DEFAULT_BOARD_SORT);
                notifyChange(
                  "Client sorting restored",
                  boardSort.category === "client"
                    ? "Clients are sorted by Date Created with the newest at the top."
                    : "The active subitem/payment sort was left unchanged.",
                );
              }}
            >
              Client only
            ...
            <AlertDialogAction
              onClick={() => {
                ...
                if (boardSort.category !== "client")
                  setBoardSort(DEFAULT_BOARD_SORT);
                notifyChange(
                  "Subitem sorting restored",
                  boardSort.category !== "client"
                    ? "Subitem and payment rows use their original order."
                    : "The active client sort was left unchanged.",
                );
              }}
            >
              Subitem only
            ...
            <AlertDialogAction
              onClick={() => {
                ...
                notifyChange(
                  ...
                  "All sorting was restored to the board defaults.",
                );
              }}
            >
              Restore all
      ...
      <AlertDialog
        open={!!pendingCloseLead}
        onOpenChange={(open) => {
          if (!open && !savingCloseLead) {
            setPendingCloseLead(null);
            setCloseLeadFiles({
              purchaseOrder: null,
              signedQuotation: null,
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Provide at least one closing document, or use a signed OCF already
              associated with this client.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="grid gap-4 py-2">
            {(() => {
              const closingClient = pendingCloseLead
                ? clients.find(
                    (client) => client.id === pendingCloseLead.clientId,
                  )
                : null;
              if (!closingClient) return null;
              return (
                <>
                  {!hasClosingQualifiedSubitem(closingClient) && (
                    <div
                      role="alert"
                      className="rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 text-sm text-amber-900"
                    >
                      <strong>Warning:</strong> This lead has no subitem that
                      has been awarded. You may still continue if this is
                      intentional.
                    </div>
                  )}
                  {!closingClient.email.trim() && (
                    <div
                      role="alert"
                      className="rounded-lg border border-red-300 bg-red-50 px-3 py-2 text-sm text-red-800"
                    >
                      <strong>Email required:</strong> Fill in the lead’s Email
                      column before it can be closed.
                    </div>
                  )}
                </>
              );
            ...
            <div
              className={`rounded-lg border px-3 py-2 text-sm font-medium ${signedOcfCheck.loading ? "border-slate-200 bg-slate-50 text-slate-600" : signedOcfCheck.signedAt ? "border-emerald-300 bg-emerald-50 text-emerald-800" : signedOcfCheck.error ? "border-amber-300 bg-amber-50 text-amber-800" : "border-slate-200 bg-slate-50 text-slate-600"}`}
            >
              {signedOcfCheck.loading ? (
                "Checking for signed OCFs…"
              ) : signedOcfCheck.signedAt ? (
                <>
                  An OCF has been signed
                  {` (${new Date(signedOcfCheck.signedAt).toLocaleDateString("en-SG")})`}
                  .
                </>
              ) : signedOcfCheck.error ? (
                "Could not check for signed OCFs. Upload a closing document to continue."
              ) : (
                "No signed OCFs found for client"
              )}
            </div>
          </div>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={savingCloseLead}>
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
              disabled={
              ...
              onClick={(event) => {
                event.preventDefault();
                void confirmCloseLead();
              }}
            >
              {savingCloseLead ? "Closing…" : "Confirm close"}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
        ...
        <div style={{ minWidth: totalMinWidth }}>
              ...
              return (
                <div
                  ...
                  onDrop={(e) => {
                    ...
                    e.preventDefault();
                    const draggedKey =
                      e.dataTransfer?.getData("text/plain") || draggedHeaderKey;
                    ...
                    <div className="flex items-center gap-1 min-w-0">
                      ...
                      {col.isCustom && col.customColumnId ? (
                        <button
                          type="button"
                          onClick={() =>
                            handleDeleteCustomColumn(col.customColumnId!)
                          }
          ...
          {(trackingView
            ...
            <React.Fragment key={group.id}>
              ...
              {(!collapsedGroups[group.id] ||
                loadedGroupContentIds.has(group.id)) &&
                !pendingGroupContentIds.has(group.id) && (
                  <div
                    ...
                    className={`relative ${collapsedGroups[group.id] ? "hidden" : ""}`}
              ...
              {(!collapsedGroups[group.id] ||
                ...
                !pendingGroupContentIds.has(group.id) && (
                  <div
                    ...
                    className={`relative h-1 ${collapsedGroups[group.id] ? "hidden" : ""}`}
                    style={{ minWidth: totalMinWidth }}
                  >
                    {groupDragOverId === group.id &&
                      groupDragOverEdge === "bottom" && (
      ...
      <GenerateOcfModal
        ...
        onSaveFinalArtwork={async (subitemId, file) => {
          ...
          await updateSubitem(ocfClient.id, subitemId, {
            customFields: {
              ...(ocfClient.subitems.find((item) => item.id === subitemId)
                ?.customFields ?? {}),
              ocfFinalArtworkFile: JSON.stringify(artwork),
            },
          });
        }}
        onCreated={() => {
      ...
      <OcfChooserModal
        ...
        onClose={() => {
          setIsOcfChooserOpen(false);
          setOcfClient(null);
        }}
        onView={() => {
          if (!ocfClient) return;
          setIsOcfChooserOpen(false);
          setDetailClientInitialTab("files");
          setDetailClientId(ocfClient.id);
          setOcfClient(null);
        }}
        onGenerate={() => {
          setIsOcfChooserOpen(false);
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <select
                  value={newColType}
                  onChange={(e) =>
                    setNewColType(e.target.value as "text" | "number" | "date")
                  }
          ...
          <div className="w-full max-w-md rounded-2xl border border-gray-200 bg-white shadow-2xl">
            <div className="border-b border-gray-100 px-5 py-4">
              ...
              <p className="mt-2 text-xs leading-5 text-gray-500">
                <span className="font-semibold text-gray-700">
                  {pendingDeleteCustomColumn.name}
                </span>{" "}
                is a shared {pendingDeleteCustomColumn.target} column. Deleting
                it will remove the column from the CRM Board for every user.
              </p>
            </div>
            <div className="flex items-center justify-end gap-2 px-5 py-4">
              <button
                type="button"
                disabled={isDeletingCustomColumn}
                onClick={() => setPendingDeleteCustomColumn(null)}
                className="rounded-xl border border-gray-200 px-3 py-2 text-sm font-medium text-gray-600 hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                type="button"
                disabled={isDeletingCustomColumn}
                onClick={() => void confirmDeleteCustomColumn()}
                className="rounded-xl bg-red-500 px-3 py-2 text-sm font-medium text-white hover:bg-red-600 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {isDeletingCustomColumn ? "Deleting..." : "Delete column"}
              </button>
            </div>
          </div>
        </div>
