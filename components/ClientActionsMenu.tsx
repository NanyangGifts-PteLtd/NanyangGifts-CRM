...
import { Copy, ExternalLink, MoreHorizontal, MoveRight, Search, Trash2 } from "lucide-react";

export function ClientActionsMenu({ clientId, clientName, groups, canEdit, onOpen, onDuplicate, onMove, onDelete, className = "", triggerClassName = "", align = "right" }: {
    clientId: string;
    clientName: string;
    ...
    return <div data-client-action-menu data-detail-action-menu={!onOpen || undefined} className={`relative ${className} ${open ? "z-[200]" : ""}`}>
        ...
        {open && <div className={`absolute top-full z-[120] mt-1 w-44 rounded-md border border-slate-200 bg-white p-1 text-left shadow-xl ${align === "left" ? "left-0" : "right-0"}`}>
            {onOpen && <button type="button" onClick={() => { setOpen(false); onOpen(); }} className="flex w-full items-center gap-2 rounded px-2 py-2 text-xs text-slate-700 hover:bg-slate-50"><ExternalLink size={14} /> Open Client</button>}
