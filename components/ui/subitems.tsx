...
const SHIPPER_PUSH_FIELDS: Array<{
  ...
  {
    key: "samples_by_air",
    label: "发样品空运 / Samples to send by air",
    required: true,
  },
  {
    key: "samples_by_sea",
...
export function SubitemsTable({
  ...
  const [dragOverColumnEdge, setDragOverColumnEdge] = useState<
    "left" | "right" | null
  >(null);
  ...
  React.useEffect(() => {
    ...
    const handler = (event: MouseEvent) => {
      ...
      if (
        target.closest(
          "[data-subitem-menu-trigger], [data-subitem-menu], [data-subitem-info-trigger], [data-subitem-info]",
        )
      )
        return;
  ...
  const [pendingPushSubitemId, setPendingPushSubitemId] = useState<
    string | null
  >(null);
  const [pushPreview, setPushPreview] = useState<ShipperPushValues | null>(
    null,
  );
  ...
  const [pushPreviewHistory, setPushPreviewHistory] = useState<{
    ...
    previousShipperName: string;
  } | null>(null);
  const [isLoadingPushPreview, setIsLoadingPushPreview] = useState(false);
  const [preparingPushSubitemId, setPreparingPushSubitemId] = useState<
    string | null
  >(null);
  ...
  const setDragPreview = (event: React.DragEvent, source: HTMLElement) => {
    ...
    Array.from(table?.tBodies[0]?.rows ?? [])
      ...
      .forEach((row, index) => {
        ...
        const cellContent = cell.firstElementChild?.cloneNode(true) as
  ...
  const reorderTableCols = (
    ...
    const next = [
      cols.find((col) => col.key === "name") ?? cols[0],
      ...reordered,
    ];
    setCols(next);
    ...
    try {
      ...
      window.dispatchEvent(
        new CustomEvent(eventName.replace("Reordered", "Changed"), {
          detail: Object.fromEntries(next.map((col) => [col.key, col.width])),
        }),
      );
    } catch {}
    ...
    if (currentUserId) {
      void import("@/lib/user-settings")
        ...
        .catch((error) =>
          console.warn("Failed to save column arrangement", error),
        );
    }
    toast.success("Column arrangement saved", {
      description: `${tableMode === "payment" ? "Payment" : "Subitem"} column order was saved to your account.`,
    });
  };
  ...
  const normalizedCurrentUserRole = String(currentUserRole ?? "")
    ...
    .toLowerCase();
  const canAccessPush =
    normalizedCurrentUserRole !== "sales" &&
    (normalizedCurrentUserRole === "pm" ||
      normalizedCurrentUserRole === "dev" ||
      normalizedCurrentUserRole === "director");
  const canCreateCustomColumns =
    normalizedCurrentUserRole === "director" ||
    normalizedCurrentUserRole === "dev";
  ...
  React.useEffect(() => {
    ...
    void fetch("/api/shipper/push", {
      ...
      .then(async (response) => {
        ...
        return Array.isArray(result?.pushedSubitemIds)
          ? result.pushedSubitemIds.map(String)
          : [];
      })
      .then((ids) => {
  ...
  const visibleCols = cols.filter(
    (col) =>
      col.key === "name" || !hiddenColumnKeys.has(`${tablePrefix}:${col.key}`),
  );
  const visibleCustomCols = subitemCustomCols.filter(
    (col) => !hiddenColumnKeys.has(`subitem:custom:${col.id}`),
  );
      ...
      if (tableMode === "payment") {
        ...
        try {
          window.localStorage.setItem(
            ...
            JSON.stringify(
              Object.fromEntries(newCols.map((c) => [c.key, c.width])),
            ),
          );
        } catch {}
        window.dispatchEvent(
      ...
      (async () => {
        try {
          ...
          const map = Object.fromEntries(
            subitemCols.map((c) => [c.key, c.width]),
          );
          try {
            localStorage.setItem(
              `colWidths:subitems:${currentUserId}`,
    ...
    const t2 = window.setTimeout(() => {
      (async () => {
        try {
          const { saveUserSetting } = await import("@/lib/user-settings");
          const map = Object.fromEntries(
            paymentCols.map((c) => [c.key, c.width]),
          );
          try {
            localStorage.setItem(
              `colWidths:payments:${currentUserId}`,
        ...
        (async () => {
          try {
            ...
            if (val && typeof val === "object") {
              setSubitemCols((prev) =>
                prev.map((c) => ({ ...c, width: val[c.key] ?? c.width })),
              );
            } else {
              try {
                const raw = localStorage.getItem(
                  `colWidths:subitems:${newUserId}`,
                );
                if (raw) {
                  const map = JSON.parse(raw) as Record<string, number>;
                  setSubitemCols((prev) =>
            ...
            if (val2 && typeof val2 === "object") {
              setPaymentCols((prev) =>
                prev.map((c) => ({ ...c, width: val2[c.key] ?? c.width })),
              );
            } else {
              try {
                const raw2 = localStorage.getItem(
                  `colWidths:payments:${newUserId}`,
                );
                if (raw2) {
                  const map2 = JSON.parse(raw2) as Record<string, number>;
                  setPaymentCols((prev) =>
                    prev.map((c) => ({ ...c, width: map2[c.key] ?? c.width })),
                  );
                }
              } catch {}
            }
  ...
  React.useEffect(() => {
    function onSubitemCols(e: any) {
      try {
        ...
        setSubitemCols((prev) =>
          prev.map((c) => ({ ...c, width: detail[c.key] ?? c.width })),
        );
      } catch {}
    }
    ...
    window.addEventListener(
      "subitemColsChanged",
      onSubitemCols as EventListener,
    );
    window.addEventListener(
      "paymentColsChanged",
      onPaymentCols as EventListener,
    );
    window.addEventListener(
      "subitemColsReordered",
      onSubitemOrder as EventListener,
    );
    window.addEventListener(
      "paymentColsReordered",
      onPaymentOrder as EventListener,
    );
    return () => {
  ...
  async function openPushPreview(subitemId: string) {
    ...
    try {
      ...
      setPushPreview({
        ...
        ...Object.fromEntries(
          SHIPPER_PUSH_FIELDS.map(({ key }) => [
            key,
            key === "info_provided_date" && !row[key]
              ? today
              : row[key] == null
                ? ""
                : String(row[key]),
      ...
      setPushPreviewHistory({
        ...
        previousShipperName: String(
          row.previous_shipper_name || "another shipper",
        ),
      });
    } catch (error: any) {
      toast.error("Could not open shipper preview", {
  ...
  React.useEffect(() => {
    const openFromSelection = (event: Event) => {
      const subitemId = (event as CustomEvent<{ subitemId?: string }>).detail
        ?.subitemId;
      if (subitemId && subitems.some((subitem) => subitem.id === subitemId))
        void openPushPreview(subitemId);
    };
    window.addEventListener("openShipperPush", openFromSelection);
    return () =>
      window.removeEventListener("openShipperPush", openFromSelection);
  }, [subitems]);

  const updatePushPreview = (field: string, value: string) =>
    setPushPreview((previous) =>
      previous ? { ...previous, [field]: value } : previous,
    );
  const pushValue = pushPreview
    ? Number(pushPreview.qty || 0) * Number(pushPreview.up || 0)
    : 0;
  const pushPreviewComplete =
    !!pushPreview &&
  ...
  const renderSubitemCell = (sub: Subitem, key: string) => {
    ...
    switch (key) {
      ...
      case "createdAt": {
        const subitemCreationActivity = clientActivityLog.find(
          (entry) =>
            entry.subitemId === sub.id && entry.action === "subitem_added",
        );
        ...
        return (
          <span
            ...
            className="block px-1 text-xs text-gray-600"
          >
            {sub.createdAt
      ...
      case "people":
        return (
          <div data-subitem-assignment-editor>
            <AssigneeMultiSelect
              profiles={profiles}
          ...
          <div className="overflow-hidden whitespace-nowrap text-ellipsis !text-center border-r border-[#D0D4E4] p-0 h-[33.1px] flex-shrink-0 transition transform active:scale-95 duration-150">
            <StatusBadge
              ...
              onUpdateOptionColor={(name, color) =>
                onUpdateOptionColor?.("local_overseas", name, color)
              }
        ...
        return (
          <div className="overflow-hidden whitespace-nowrap text-ellipsis !text-center border-r border-[#D0D4E4] p-0 h-[33.1px] flex-shrink-0 transition transform active:scale-95 duration-150">
            <StatusBadge
              ...
              onUpdateOptionColor={(name, color) =>
                onUpdateOptionColor?.("subitem_status", name, color)
              }
              onRenameOption={(oldName, newName) =>
        ...
        return (
          <div className="overflow-hidden whitespace-nowrap text-ellipsis !text-center border-r border-[#D0D4E4] p-0 h-[33.1px] flex-shrink-0 transition transform active:scale-95 duration-150">
            <StatusBadge
              ...
              manageLabel="shipper"
              onUpdateOptionColor={(name, color) =>
                onUpdateOptionColor?.("shipper", name, color)
              }
              onRenameOption={(oldName, newName) =>
        ...
        return (
          <div className="overflow-hidden whitespace-nowrap text-ellipsis !text-center border-r border-[#D0D4E4] p-0 h-[33.1px] flex-shrink-0 transition transform active:scale-95 duration-150">
            <StatusBadge
              ...
              onUpdateOptionColor={(name, color) =>
                onUpdateOptionColor?.("currency", name, color)
              }
              onRenameOption={(oldName, newName) =>
      ...
      case "cSgd":
        return (
          <div className="flex justify-center text-xs text-gray-800">
            {hasCurrency ? formatMoney(cSgd) : ""}
          </div>
        );
      case "tc":
        return (
          <div className="flex justify-center text-xs text-gray-800">
            {hasCurrency ? formatMoney(tc) : ""}
          </div>
        );
      case "uc":
        return (
          <div className="flex justify-center text-xs text-gray-800">
            {hasCurrency ? formatMoney(uc) : ""}
          </div>
        );
      case "pl":
        return (
          <EditableCell
            value={sub.pl ?? ""}
            onChange={(v) => onUpdateSubitem(sub.id, { pl: v })}
            type="number"
          />
        );
      case "sl":
        return (
          <EditableCell
            value={sub.sl ?? ""}
            onChange={(v) => onUpdateSubitem(sub.id, { sl: v })}
            type="number"
          />
        );
      case "tcSgd":
        return (
          <div className="flex justify-center text-xs text-gray-800">
            {hasCurrency ? formatMoney(tcSgd) : ""}
          </div>
        );
      case "price":
        return (
          <div className="flex justify-center text-xs text-gray-800">
            {hasCurrency ? formatMoney(price) : ""}{" "}
          </div>
        );
      case "up":
        return (
      ...
      case "markup":
        return (
          <div className="flex justify-center text-xs text-gray-800">
            {hasCurrency ? formatMoney(markup) : ""}
          </div>
        );
      case "percentMarkup":
        return (
          <div className="flex justify-center text-xs text-gray-800">
            {hasCurrency && percentMarkup !== null
              ? `${formatMoney(percentMarkup)}%`
              : ""}
          </div>
        );
      case "idealMarkup":
  ...
  const renderPaymentCell = (sub: Subitem, key: string) => {
    ...
    switch (key) {
        ...
        return (
          <div className="overflow-hidden whitespace-nowrap text-ellipsis !text-center border-r border-[#D0D4E4] p-0 h-[33.1px] flex-shrink-0 transition transform active:scale-95 duration-150">
            <StatusBadge
              ...
              manageLabel="payment"
              onUpdateOptionColor={(name, color) =>
                onUpdateOptionColor?.("payment", name, color)
              }
              onRenameOption={(oldName, newName) =>
      ...
      case "totalUc":
        return (
          <div className="flex justify-center text-xs text-gray-800">
            {hasCurrency ? formatMoney(totalUc) : ""}
          </div>
        );
      case "manpower":
        return (
      ...
      case "totalC":
        return (
          <div className="flex justify-center text-xs text-gray-800">
            {hasCurrency ? formatMoney(totalC) : ""}
          </div>
        );
      case "modeOfPayment":
        return (
          <div className="overflow-hidden whitespace-nowrap text-ellipsis !text-center border-r border-[#D0D4E4] p-0 h-[33.1px] flex-shrink-0 transition transform active:scale-95 duration-150">
            <StatusBadge
              ...
              onUpdateOptionColor={(name, color) =>
                onUpdateOptionColor?.("mode_of_payment", name, color)
              }
              onRenameOption={(oldName, newName) =>
      ...
      case "orderNumber":
        return (
          <EditableCell
            value={sub.orderNumber}
            onChange={(v) => onUpdateSubitem(sub.id, { orderNumber: v })}
          />
        );
  ...
  const renderCell = (sub: Subitem, key: string) =>
    tableMode === "payment"
      ? renderPaymentCell(sub, key)
      : renderSubitemCell(sub, key);

  const updateTimelineRowsWithDependencies = (
    ...
    if (automaticUpdates > 0) {
      toast.success("Timeline dates updated", {
        ...
        action: {
          label: "Details",
          onClick: () =>
            toast("Dependency automation", {
              description:
                "A dependent process starts one day after the selected dependency process ends.",
            }),
        },
      });
    }
  ...
  return (
    ...
    >
      ...
      <AlertDialog
        open={!!pendingPushSubitemId}
        onOpenChange={(open) => !open && setPendingPushSubitemId(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
      ...
      {pushPreview && (
        ...
        >
          <div className="flex max-h-[calc(100vh-2rem)] w-full max-w-4xl flex-col overflow-hidden rounded-xl bg-white shadow-2xl">
            ...
            {pushPreviewHistory?.alreadyPushed && (
              <div
            ...
            <div className="flex-1 space-y-4 overflow-y-auto p-5">
              <div className="grid gap-4 border-b border-slate-200 pb-4 md:grid-cols-2">
                {pushField("info_provided_date", "Date of Submission")}
                {pushField("cn_tracking_no", "CN Tracking")}
              </div>
              <div className="grid gap-4 md:grid-cols-3">
                <div className="space-y-4 md:border-r md:border-slate-200 md:pr-4">
                  {pushField("qty", "Qty")}
                  {pushField("up", "Unit Price")}
                  <div className="text-xs font-medium text-slate-700">
                    Value
                    <div className="mt-1 rounded-md border border-slate-200 bg-slate-100 px-3 py-2 text-sm">
                      {Number.isFinite(pushValue)
                        ? pushValue.toFixed(2)
                        : "0.00"}
                    </div>
                  </div>
                  {pushField("cartons", "Cartons")}
                  {pushField("tax_refund", "退税?")}
                </div>
                <div className="space-y-4 md:border-r md:border-slate-200 md:px-4">
                  {pushField("delivery_info", "Address")}
                  {pushField("sea_or_air", "Air/Sea?")}
                </div>
            ...
            <div className="flex items-center justify-between border-t border-slate-200 px-5 py-4">
              ...
              <div className="flex gap-2">
                ...
                <button
                  type="button"
                  onClick={() => void confirmPushPreview()}
                  disabled={
                    !pushPreviewComplete ||
          ...
          <div className="w-full max-w-2xl rounded-xl bg-white p-4 shadow-xl">
            ...
            <div className="max-h-[420px] space-y-2 overflow-y-auto">
              {clientActivityLog.filter(
                ...
                clientActivityLog
                  .filter((entry) => entry.subitemId === activitySubitem.id)
                  .map((entry) => {
                    const fieldName = entry.fieldName ?? "";
                    const timelineMatch = fieldName.match(
                      /^timeline:\s*(.*?):(.*)$/,
                    );
                    ...
                    return (
                      <div
                        key={entry.id}
                        className="flex items-start justify-between gap-3 rounded-lg border border-gray-100 bg-gray-50 px-3 py-2 text-xs"
                      >
                        <div>
                          <div className="text-gray-800">
                            {entry.title ? (
                              ...
                              <>
                                parent client changed from{" "}
                                <span className="font-medium">
                                  {displayActivityValue(entry.oldValue)}
                                </span>{" "}
                                to{" "}
                                <span className="font-medium">
                                  {displayActivityValue(entry.newValue)}
                                </span>
                              </>
                            ) : timelineMatch ? (
                              <>
                                ...
                                from {displayActivityValue(entry.oldValue)} to{" "}
                                {displayActivityValue(entry.newValue)}
                              </>
                            ) : (
                              <>
                                {entry.fieldName || entry.action}:{" "}
                                {displayActivityValue(entry.oldValue)}{" "}
            ...
            <tr className="border-b border-t border-r border-[#D0D4E4] bg-gray-50">
                ...
                return (
                  <th
                    ...
                    onDragStart={(e) => {
                      ...
                      e.dataTransfer?.setData(
                        "application/x-crm-table-column",
                        col.key,
                      );
                  ...
                  >
                    <div className="flex items-center justify-center gap-1 overflow-hidden px-2">
                      ...
                      {tablePrefix === "subitem" &&
                        SUBITEM_COLUMN_DESCRIPTIONS[col.key] && (
                          <button
                            type="button"
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              setOpenColumnInfo(
                                openColumnInfo === col.key ? null : col.key,
                              );
                            }}
                    ...
                    {openColumnMenu === `${tablePrefix}:${col.key}` && (
                      <div
                        data-subitem-menu
                        className="absolute left-0 top-full z-[80] mt-1 w-36 rounded-md border border-gray-200 bg-white p-1 text-left shadow-xl"
                      >
                        {((tablePrefix === "subitem" &&
                          ["people", "status"].includes(col.key)) ||
                          (tablePrefix === "payment" &&
                            ["payment", "paymentStatus"].includes(
                              col.key,
                            ))) && (
                          <button
                            type="button"
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            onSortColumn?.(tablePrefix, col.key, "asc");
                            setOpenColumnMenu(null);
                          }}
                          className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-[10px] font-medium text-gray-700 hover:bg-gray-50"
                        >
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            onSortColumn?.(tablePrefix, col.key, "desc");
                            setOpenColumnMenu(null);
                          }}
                          className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-[10px] font-medium text-gray-700 hover:bg-gray-50"
                        >
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            onHideColumn(`${tablePrefix}:${col.key}`)
                          }
                          className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-[10px] font-medium text-gray-700 hover:bg-gray-50"
                        >
                    ...
                    {dragOverColumnKey === col.key && isDragTarget && (
                      <div
                        className={`pointer-events-none absolute inset-y-0 z-20 w-1 bg-[#0f8da8] shadow-[0_0_5px_rgba(15,141,168,0.6)] ${dragOverColumnEdge === "left" ? "left-0" : "right-0"}`}
                      />
                    )}
                    <div
                      onMouseDown={(e) => {
              ...
              {visibleCustomCols.map((col) => (
                <th
                  ...
                  onContextMenu={(e) => {
                    e.preventDefault();
                    setOpenColumnMenu(`${tablePrefix}:custom:${col.id}`);
                  }}
                  ...
                  style={{
                    minWidth: CUSTOM_COL_WIDTH,
                    width: CUSTOM_COL_WIDTH,
                  }}
                >
                  ...
                  <button
                    ...
                    className="absolute right-0.5 top-0.5 z-30 hidden rounded bg-white/90 p-0.5 text-gray-400 shadow-sm hover:text-gray-700 group-hover:block"
                    title={`Column options for ${col.name}`}
                  >
                    <MoreHorizontal size={12} />
                  </button>
                  {openColumnMenu === `${tablePrefix}:custom:${col.id}` && (
                    <div
                      data-subitem-menu
                    ...
                    >
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          onHideColumn(`${tablePrefix}:custom:${col.id}`)
                        }
                        className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-[10px] font-medium text-gray-700 hover:bg-gray-50"
                      >
                        <EyeOff size={12} /> Hide column
              ...
              <th
                className="border-r border-[#D0D4E4] px-1 text-center"
                style={{ width: 32 }}
              >
              ...
              <React.Fragment key={sub.id}>
                <tr
                  ...
                  onContextMenu={(event) => {
                    event.preventDefault();
                    window.dispatchEvent(
                      new CustomEvent("crm:subitem-actions", {
                        detail: sub.id,
                      }),
                    );
                  }}
                  ...
                  onClickCapture={(event) => {
                    ...
                    if (
                      isEditControl &&
                      !target.closest(
                        "[data-subitem-assignment-editor], [data-view-action], [data-selection-control], [data-subitem-action-menu]",
                      )
                    ) {
                      event.preventDefault();
                ...
                >
                  <td className="group relative overflow-visible border-r border-[#D0D4E4] px-2 py-1 text-center">
                    <SubitemActionsMenu
                      ...
                      canEdit={canEditSubitem(sub.id)}
                      onOpen={() => onOpenSubitemDetail?.(sub.id)}
                      onDuplicate={() => onDuplicateSubitemAction(sub.id)}
                  ...
                  <td
                    className="border-r border-[#D0D4E4] text-center"
                    style={{ width: 32 }}
                  >
                    <button
                      type="button"
            ...
            {pendingSubitemName && (
              ...
              >
                ...
                {visibleCols.map((col) => (
                  <td
                    key={col.key}
                    className="h-[33px] border-r border-[#D0D4E4] px-2 text-xs"
                  >
                    {col.key === "name" ? (
                      ...
                      <span className="block h-2 w-2/3 animate-pulse rounded bg-gray-100" />
                    )}
                  </td>
                ))}
                {visibleCustomCols.map((col) => (
                  <td key={col.id} className="border-r border-[#D0D4E4] px-2">
                    <span className="block h-2 w-2/3 animate-pulse rounded bg-gray-100" />
                  </td>
                ))}
