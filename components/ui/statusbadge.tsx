...
const LABEL_COLORS = [
    "#b91c1c", "#dc2626", "#ef4444", "#f43f5e", "#be123c", "#ec4899",
    "#db2777", "#c026d3", "#a855f7", "#9333ea", "#7c3aed", "#6366f1",
    "#4f46e5", "#2563eb", "#3b82f6", "#0ea5e9", "#0891b2", "#06b6d4",
    "#0d9488", "#14b8a6", "#059669", "#16a34a", "#22c55e", "#65a30d",
    "#84cc16", "#ca8a04", "#eab308", "#f59e0b", "#ea580c", "#f97316",
    "#78716c", "#64748b", "#475569", "#334155", "#94a3b8", "#d1d5db",
];
...
export function StatusBadge({
    ...
    const handleOpen = () => {
        ...
        const estimatedHeight = Math.min(Math.ceil((options.length + 1) / 3) * 46 + 76, 520);
        const top = window.innerHeight - rect.bottom >= estimatedHeight ? rect.bottom + 6 : Math.max(8, rect.top - estimatedHeight - 6);
    ...
    const rename = async (oldName: string) => {
        ...
        if (nextName && nextName !== oldName) await onRenameOption?.(oldName, nextName);
    };

    const menu = open && createPortal(
        <div ref={menuRef} style={menuStyle} className="max-h-[min(520px,calc(100vh-16px))] overflow-y-auto rounded-xl border border-gray-200 bg-white p-4 shadow-2xl">
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                {options.map((option) => {
                    ...
                    return <div key={option.value || "__empty__"} className="relative min-w-0">
                        ...
                        {colorEditor === option.value && onUpdateOptionColor && <div className="absolute left-0 top-10 z-20 grid w-56 grid-cols-6 gap-2 rounded-lg border border-gray-200 bg-white p-2 shadow-xl">{LABEL_COLORS.map((color) => <button key={color} type="button" onClick={async () => { await onUpdateOptionColor(option.value, color); setColorEditor(null); }} className="h-6 w-6 rounded-md border border-white ring-1 ring-gray-200 transition hover:scale-110" style={{ background: color }} title={color} />)}</div>}
                    </div>;
                })}
                {editingLabels && onAddOption && <div className="flex h-9 items-center gap-1 rounded-md border border-dashed border-gray-300 p-1"><input value={newOption} onChange={(event) => setNewOption(event.target.value)} placeholder={`New ${manageLabel}`} className="min-w-0 flex-1 px-2 text-xs text-gray-700 outline-none" /><button type="button" onClick={async () => { const name = newOption.trim(); if (!name) return; await onAddOption(name); setNewOption(""); }} className="inline-flex h-7 items-center rounded bg-[#7BCBD5] px-2 text-[10px] font-semibold text-white"><Plus size={11} /> Add</button></div>}
            </div>
            {(onAddOption || onUpdateOptionColor || onRenameOption) && <button type="button" onClick={() => { setEditingLabels((previous) => !previous); setColorEditor(null); }} className="mt-4 flex w-full items-center justify-center gap-2 border-t border-gray-200 pt-3 text-sm text-gray-600 hover:text-gray-900"><Pencil size={15} />{editingLabels ? "Done editing" : "Edit labels"}</button>}
        </div>, document.body
    );
