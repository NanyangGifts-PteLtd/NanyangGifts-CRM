...
export function EditableCell({
  ...
  useEffect(() => {
    if (!editing || !multiline || resizableMultiline || !textareaRef.current)
      return;
          ...
          <div className="absolute left-0 top-[264px] z-[1001] flex w-[min(620px,calc(100vw-48px))] flex-wrap gap-1 rounded-b border border-slate-200 bg-white p-2 shadow-lg">
            {matches.map((item) => (
              <button
                ...
                className="rounded bg-sky-50 px-2 py-1 text-xs text-sky-700 hover:bg-sky-100"
              >
                Use closest match: {item}
              </button>
            ))}
          </div>
        )}
