...
import { ChevronLeft, ChevronRight, FileText, MoreHorizontal, Paperclip, X } from "lucide-react";
...
type Attachment = { id: string; name: string; url: string; kind?: string };
...
export function ClientDetailView({ client, clients, profiles, assigneeIds, pmIds, canEdit, onClose, onNavigate, onUpdate, onChangeAssignees, onUndo, statusOptions, replyStatusOptions, channelOptions, importanceOptions, groupNamesById }: {
  ...
  const ocfItems = (client.activityLog ?? []).filter((entry) => entry.action === "ocf_created" && entry.link).map((entry) => ({ id: entry.id, name: entry.title || "Order confirmation", url: entry.link!, actorName: entry.actorName, createdAt: entry.createdAt }));
  ...
  return <div className="fixed inset-0 z-[200] bg-slate-950/40 p-3 sm:p-6">
    <section className="flex h-full flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
      ...
      <nav className="flex gap-6 border-b border-slate-200 px-6">{([['overview', 'Overview'], ['files', 'Files'], ['activity', 'Activity Log']] as const).map(([key, label]) => <button key={key} onClick={() => setTab(key)} className={`border-b-2 py-3 text-sm ${tab === key ? "border-sky-500 text-sky-600" : "border-transparent text-slate-500"}`}>{label}</button>)}</nav>
