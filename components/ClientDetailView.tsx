...
import { Building2, ChevronLeft, ChevronRight, FileText, Paperclip, Send, Trash2, UserRound, X } from "lucide-react";
...
type RelatedProfileData = {
  clients: Array<{ id: string; phone_number: string; phone_numbers?: Array<{ phone_number: string }> }>;
  companies: Array<{ id: string; name: string }>;
  links: Array<{ client_id: string; client_profile_id: string | null; company_profile_id: string | null }>;
};

const normalizeRelatedPhone = (value: string) => value.replace(/\D/g, "");
const normalizeRelatedCompany = (value: string) => value.trim().replace(/\s+/g, " ").toLocaleLowerCase();
...
export function ClientDetailView({ client, clients, profiles, assigneeIds, pmIds, canEdit, currentUserId, currentUserRole, groups, onDuplicate, onMove, onDelete, onClose, onNavigate, onUpdate, onChangeAssignees, onUndo, statusOptions, replyStatusOptions, channelOptions, importanceOptions, groupNamesById, initialTab }: {
  ...
  useEffect(() => {
    ...
    void fetch("/api/customer-profiles").then(async (response) => {
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || "Unable to load related leads");
      if (active) setRelatedData(result as RelatedProfileData);
    }).catch(() => { if (active) setRelatedData(null); }).finally(() => { if (active) setRelatedLoading(false); });
    return () => { active = false; };
  }, [client.id, tab]);
  const relatedByClient = useMemo(() => {
    if (!relatedData) return [];
    const currentLink = relatedData.links.find((link) => link.client_id === client.id);
    const phone = normalizeRelatedPhone(client.phone ?? "");
    const profileId = currentLink?.client_profile_id ?? relatedData.clients.find((profile) => [profile.phone_number, ...(profile.phone_numbers ?? []).map((item) => item.phone_number)].some((value) => normalizeRelatedPhone(value) === phone))?.id;
    const linkedIds = new Set(relatedData.links.filter((link) => profileId && link.client_profile_id === profileId).map((link) => link.client_id));
    return clients.filter((lead) => lead.id !== client.id && (linkedIds.has(lead.id) || Boolean(phone && normalizeRelatedPhone(lead.phone ?? "") === phone)));
  ...
  const relatedByCompany = useMemo(() => {
    ...
    const profileId = currentLink?.company_profile_id ?? relatedData.companies.find((profile) => normalizeRelatedCompany(profile.name) === company)?.id;
    const linkedIds = new Set(relatedData.links.filter((link) => profileId && link.company_profile_id === profileId).map((link) => link.client_id));
    return clients.filter((lead) => lead.id !== client.id && (linkedIds.has(lead.id) || Boolean(company && normalizeRelatedCompany(lead.company ?? "") === company)));
  ...
  return <div className="fixed inset-0 z-[200] bg-slate-950/40 p-3 sm:p-6">
    <section className="flex h-full flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
      <header className="flex items-center gap-4 border-b border-slate-200 px-6 py-4">
        ...
        <ClientActionsMenu clientId={client.id} clientName={client.name} groups={groups} canEdit={canEdit} onDuplicate={onDuplicate} onMove={onMove} onDelete={onDelete} className="shrink-0" /><button onClick={onClose} className="rounded border p-2" title="Close"><X size={17} /></button>
      </header>
      <nav className="flex gap-6 border-b border-slate-200 px-6">{([['overview', 'Overview'], ['updates', 'Updates'], ['files', 'Files'], ['activity', 'Activity Log'], ['related', 'Related Leads']] as const).map(([key, label]) => <button key={key} onClick={() => setTab(key)} className={`border-b-2 py-3 text-sm ${tab === key ? "border-sky-500 text-sky-600" : "border-transparent text-slate-500"}`}>{label}</button>)}</nav>
      ...
      {tab === "files" && <main className="min-h-0 flex-1 overflow-auto bg-slate-50 p-6">{[["OCF files", "ocfFiles", ocfItems] as const, ...fileGroups.map(([label, field]) => [label, field, attachments(client.customFields?.[field], field)] as const)].map(([label, field, items]) => <FileDropTarget key={field} disabled={!canEdit || field === "ocfFiles"} onFiles={(files) => { void attachmentsFromFiles(files).then((next) => saveFiles(field, [...items, ...next])); }}><section className="mb-5 rounded-xl border border-slate-200 bg-white p-5"><div className="mb-3 flex items-center justify-between"><h2 className="font-semibold text-slate-800">{label}</h2>{canEdit && field !== "ocfFiles" && <label className="cursor-pointer rounded border px-3 py-1.5 text-xs text-sky-700"><Paperclip size={13} className="mr-1 inline" />Add file<input type="file" className="hidden" onChange={(event) => { void attachmentsFromFiles(Array.from(event.target.files ?? [])).then((next) => saveFiles(field, [...items, ...next])); event.target.value = ""; }} /></label>}</div>{items.length ? <div className="space-y-2">{items.map((item) => <div key={item.id} className="flex items-center gap-2 rounded border p-2"><FileText size={16} className="text-sky-600" /><div className="min-w-0 flex-1"><a href={item.url} target="_blank" rel="noreferrer" className="block truncate text-sm text-sky-700 hover:underline">{item.name}</a>{item.signed && <span className="mt-1 inline-flex rounded-full bg-emerald-100 px-2 py-0.5 text-[11px] font-semibold text-emerald-700">Signed</span>}{item.createdAt && <p className="text-xs text-slate-400">{item.createdThrough ? `${item.createdThrough} · ` : ""}{item.actorName} - {new Date(item.createdAt).toLocaleString("en-GB")}</p>}</div>{canEdit && field !== "ocfFiles" && <button onClick={() => saveFiles(field, items.filter((entry) => entry.id !== item.id))} className="text-xs text-red-500">Remove</button>}</div>)}</div> : <p className="text-sm text-slate-400">No files attached.</p>}</section></FileDropTarget>)}</main>}
      ...
      {tab === "related" && <main className="min-h-0 flex-1 overflow-auto bg-slate-50 p-6">
        <div className="mx-auto grid max-w-6xl gap-5 lg:grid-cols-2">
          <RelatedLeadSection title="Related leads by client" description={client.phone ? `Matched through ${client.phone}` : "Add a phone number to match client profiles."} icon={<UserRound size={19} />} leads={relatedByClient} detail={(lead) => lead.phone || "No phone number"} loading={relatedLoading} onOpen={onNavigate} />
          <RelatedLeadSection title="Related leads by company" description={client.company ? `Matched through ${client.company}` : "Add a company name to match company profiles."} icon={<Building2 size={19} />} leads={relatedByCompany} detail={(lead) => lead.company || "No company"} loading={relatedLoading} onOpen={onNavigate} />
        </div>
      </main>}
      {tab === "activity" && <main className="min-h-0 flex-1 overflow-auto bg-slate-50 p-6"><div className="mx-auto max-w-4xl space-y-3">{clientActivities.map((entry) => <article key={entry.id} className="rounded-lg border border-slate-100 bg-white px-3 py-2"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-sm text-slate-800"><span className="font-medium">{entry.actorName}</span> {activityText(entry)}{entry.link && <a href={entry.link} target="_blank" rel="noreferrer" className="ml-4 inline-flex items-center rounded-md bg-teal-100 px-2 py-1 text-[12.6px] font-medium text-teal-600 hover:bg-teal-200">{entry.action === "estimate_created" || String(entry.meta?.fileName ?? "").startsWith("Sample Estimate") ? "Open Estimate" : "Open OCF"}</a>}{entry.description === "File has been removed" && <span className="ml-4 inline-flex items-center rounded-md bg-slate-100 px-2 py-1 text-[12.6px] font-medium text-slate-500" title="This file is no longer available">File has been removed</span>}{entry.description === "File has been replaced" && <span className="ml-4 inline-flex items-center rounded-md bg-slate-100 px-2 py-1 text-[12.6px] font-medium text-slate-500" title="This file has been replaced">File has been replaced</span>}</p><p className="mt-1 text-[12.6px] text-slate-500">{new Date(entry.createdAt).toLocaleString("en-GB")}</p></div>{(entry.action === "field_changed" || entry.action === "subitem_field_changed") && entry.oldValue != null && <button type="button" disabled={!canEdit} onClick={() => void onUndo(entry)} title={!canEdit ? "You can only edit items that are assigned to you" : "Undo this action"} className="shrink-0 rounded-md border border-gray-200 bg-white px-2 py-1 text-[11px] font-medium text-gray-600 hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-50">Undo</button>}</div></article>)}{!clientActivities.length && <p className="text-center text-sm text-slate-400">No activity recorded yet.</p>}</div></main>}
    </section>
...
function RelatedLeadSection({ title, description, icon, leads, detail, loading, onOpen }: { title: string; description: string; icon: React.ReactNode; leads: Client[]; detail: (lead: Client) => string; loading: boolean; onOpen: (lead: Client) => void }) {
  return <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
    <header className="flex items-start gap-3 border-b border-slate-200 px-5 py-4"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-sky-50 text-sky-600">{icon}</span><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-3"><h2 className="font-semibold text-slate-800">{title}</h2><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-600">{leads.length}</span></div><p className="mt-0.5 truncate text-xs text-slate-500">{description}</p></div></header>
    <div className="divide-y divide-slate-100">{loading ? <p className="px-5 py-10 text-center text-sm text-slate-400">Loading related leads...</p> : leads.map((lead) => <button key={lead.id} type="button" onClick={() => onOpen(lead)} className="flex w-full items-center gap-3 px-5 py-4 text-left transition hover:bg-sky-50/60 focus:bg-sky-50/60 focus:outline-none"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-slate-100 text-xs font-semibold text-slate-600">{lead.name.trim().slice(0, 2).toUpperCase() || "?"}</span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold text-slate-800">{lead.name || "Unnamed lead"}</span><span className="mt-0.5 block truncate text-xs text-slate-500">{detail(lead)}</span></span><span className="rounded-full bg-slate-100 px-2 py-1 text-[11px] font-medium text-slate-600">{lead.status || "No status"}</span><ChevronRight size={17} className="shrink-0 text-slate-300" /></button>)}{!loading && !leads.length && <p className="px-5 py-10 text-center text-sm text-slate-400">No related leads found.</p>}</div>
  </section>;
}
