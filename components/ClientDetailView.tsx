...
const normalizeRelatedCompany = (value: string) =>
...
export function ClientDetailView({
  ...
  const [relatedData, setRelatedData] = useState<RelatedProfileData | null>(
    null,
  );
  ...
  const people = useMemo(
    () =>
      [...new Set([...assigneeIds, ...pmIds])]
        .map((id) => profiles.find((profile) => profile.id === id))
        .filter(Boolean) as Profile[],
    [assigneeIds, pmIds, profiles],
  );
  const pmProfiles = profiles.filter(
    (profile) => profile.role?.toLowerCase() === "pm",
  );
  const textFields: Array<[string, keyof Client]> = [
    ["Company", "company"],
    ["Email", "email"],
    ["Phone", "phone"],
  ...
  const saveFiles = (field: string, next: Attachment[]) =>
    onUpdate({
      customFields: {
        ...(client.customFields ?? {}),
        [field]: JSON.stringify(next),
      },
    });
  const signedOcfIds = new Set(
    (client.activityLog ?? [])
      .filter((entry) => entry.action === "ocf_signed")
      .map((entry) => String(entry.meta?.ocfId ?? ""))
      .filter(Boolean),
  );
  const activityOcfItems: Attachment[] = (client.activityLog ?? [])
    .filter((entry) => entry.action === "ocf_created" && entry.link)
    .map((entry) => ({
      id: entry.id,
      name: entry.title || "Order confirmation",
      url: entry.link!,
      actorName: entry.actorName,
      createdAt: entry.createdAt,
      signed: signedOcfIds.has(String(entry.meta?.ocfId ?? "")),
    }));
  ...
  useEffect(() => {
    ...
    let active = true;
    void fetch(
      `/api/order-confirmations?clientId=${encodeURIComponent(client.id)}`,
    )
      .then(async (response) => {
      ...
      .then((items) => {
        if (active) setOcfFiles(items);
      })
      .catch(() => {
        if (active) setOcfFiles([]);
      });
    return () => {
      active = false;
    };
  }, [client.id, tab, client.activityLog]);
  const displayValue = (field: string | undefined, value: unknown) =>
    field === "groupId" && typeof value === "string"
      ? (groupNamesById[value] ?? "Ungrouped")
      : String(value ?? "empty");
  const activityText = (entry: ActivityEntry) =>
    entry.title ||
    (entry.fieldName
  ...
  const showPermissionNotice = (element: HTMLElement) => {
    ...
    setPermissionNotice({
      ...
      y: Math.min(rect.bottom + 8, window.innerHeight - 54),
    });
  };
  ...
  const mentionQuery =
    updateDraft.match(/(?:^|\s)@([^@\s]*)$/)?.[1]?.toLowerCase() ?? null;
  ...
  const loadUpdates = async () => {
    ...
    try {
      const response = await fetch(
        `/api/client-updates?clientId=${encodeURIComponent(client.id)}`,
      );
  ...
  useEffect(() => {
    ...
    void fetch("/api/customer-profiles")
      .then(async (response) => {
        ...
        if (!response.ok)
          throw new Error(result.error || "Unable to load related leads");
        if (active) setRelatedData(result as RelatedProfileData);
      })
      .catch(() => {
        if (active) setRelatedData(null);
      })
      .finally(() => {
  ...
  const relatedByClient = useMemo(() => {
    ...
    const profileId =
      ...
      relatedData.clients.find((profile) =>
        [
    ...
    const linkedIds = new Set(
      relatedData.links
        .filter((link) => profileId && link.client_profile_id === profileId)
        .map((link) => link.client_id),
    );
    return clients.filter(
      (lead) =>
        lead.id !== client.id &&
        (linkedIds.has(lead.id) ||
          Boolean(phone && normalizeRelatedPhone(lead.phone ?? "") === phone)),
    );
  ...
  const relatedByCompany = useMemo(() => {
    ...
    const profileId =
      ...
      relatedData.companies.find(
        (profile) => normalizeRelatedCompany(profile.name) === company,
      )?.id;
    const linkedIds = new Set(
      relatedData.links
        .filter((link) => profileId && link.company_profile_id === profileId)
        .map((link) => link.client_id),
    );
    return clients.filter(
      (lead) =>
        ...
        (linkedIds.has(lead.id) ||
          Boolean(
            company && normalizeRelatedCompany(lead.company ?? "") === company,
          )),
    );
  ...
  const selectMention = (profile: Profile) => {
    ...
    setMentionIds((ids) =>
      ids.includes(profile.id) ? ids : [...ids, profile.id],
    );
  };
  ...
  const deleteUpdate = async (id: string) => {
    const response = await fetch(
      `/api/client-updates?id=${encodeURIComponent(id)}`,
      { method: "DELETE" },
    );
    if (response.ok)
    ...
    <div className="fixed inset-0 z-[200] bg-slate-950/40 p-3 sm:p-6">
      <section className="flex h-full flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
        <header className="flex items-center gap-4 border-b border-slate-200 px-6 py-4">
          ...
          <button
            onClick={() => index > 0 && onNavigate(clients[index - 1])}
            disabled={index <= 0}
          ...
          <button
            onClick={() =>
              index < clients.length - 1 && onNavigate(clients[index + 1])
            }
          ...
          <main className="grid min-h-0 flex-1 grid-cols-1 gap-4 overflow-auto bg-slate-50 p-4 lg:grid-cols-[1fr_720px]">
            <div className="flex min-h-80 items-center justify-center rounded-xl border border-slate-200 bg-white text-center text-slate-400">
              <div>
                ...
                <p>Activity timeline placeholder</p>
              </div>
            </div>
            <aside className="rounded-xl border border-slate-200 bg-white p-7">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-5">
                <DetailStatus
                  ...
                  locked={!canEdit}
                  onLockedClick={showPermissionNotice}
                  onChange={(replyStatus) => onUpdate({ replyStatus })}
                />
                <DetailStatus
                  label="Status"
                  value={client.status}
                  ...
                  onLockedClick={showPermissionNotice}
                  onChange={(status) =>
                    onUpdate({ status: status as Client["status"] })
                  }
                />
                <DetailStatus
                  label="Channel"
                  ...
                  >
                    ...
                    >
                      <EditableCell
                        ...
                        onChange={(value) =>
                          onUpdate({ [key]: value } as Partial<Client>)
                        }
                      />
                    </div>
                  </label>
                ...
                {dateFields.map(([label, key]) => (
                  ...
                  >
                    ...
                    <div
                      onClickCapture={blockIfLocked}
                      className="mt-2 flex min-h-10 items-center rounded border border-slate-200 bg-white px-3"
                    >
                      {canEdit ? (
                        <input
                          type="date"
                          value={dateInputValue(client[key])}
                          onChange={(event) =>
                            onUpdate({
              ...
              ...fileGroups.map(
                ([label, field]) =>
                  [
                    label,
                    field,
                    attachments(client.customFields?.[field], field),
                  ] as const,
              ),
            ...
            <div className="mx-auto max-w-3xl">
              <div className="relative rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                <textarea
                  ...
                  placeholder="Write an update… Type @ to tag a teammate."
                  rows={3}
                  className="w-full resize-none border-0 text-sm text-slate-800 outline-none"
                />
                {mentionCandidates.length > 0 && (
                  <div className="absolute left-4 top-full z-10 mt-1 w-72 overflow-hidden rounded-lg border border-slate-200 bg-white py-1 shadow-xl">
                ...
                <div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-3">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-md bg-sky-600 px-3 py-2 text-sm font-medium text-white hover:bg-sky-700 disabled:opacity-50"
                  >
                    <Send size={14} />
                ...
                {updatesLoading ? (
                  ...
                  updates.map((update) => {
                    ...
                    const canDelete =
                      ...
                      ["director", "dev"].includes(currentUserRole ?? "");
                    return (
                      <article
                        key={update.id}
                      ...
                      >
                        <div className="flex gap-3">
                          <span
                            ...
                            style={{
                              background: gradientForId(update.author_id),
                            }}
                          >
                            {(author?.full_name || author?.email || "U")
                              .slice(0, 2)
                              .toUpperCase()}
                          </span>
                          <div className="min-w-0 flex-1">
          ...
          <main className="min-h-0 flex-1 overflow-auto bg-slate-50 p-6">
            <div className="mx-auto grid max-w-6xl gap-5 lg:grid-cols-2">
              <RelatedLeadSection
                ...
                description={
                  client.phone
                    ? `Matched through ${client.phone}`
                    : "Add a phone number to match client profiles."
                }
                icon={<UserRound size={19} />}
                leads={relatedByClient}
                detail={(lead) => lead.phone || "No phone number"}
                loading={relatedLoading}
                onOpen={onNavigate}
              />
              <RelatedLeadSection
                title="Related leads by company"
                description={
                  client.company
                    ? `Matched through ${client.company}`
                    : "Add a company name to match company profiles."
                }
                icon={<Building2 size={19} />}
                leads={relatedByCompany}
                detail={(lead) => lead.company || "No company"}
                loading={relatedLoading}
                onOpen={onNavigate}
              />
            </div>
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 text-[12.6px] text-slate-500">
...
function RelatedLeadSection({
  ...
  leads: Client[];
  detail: (lead: Client) => string;
  loading: boolean;
  onOpen: (lead: Client) => void;
}) {
  return (
...
function DetailStatus({
  ...
  locked: boolean;
  onLockedClick: (element: HTMLElement) => void;
}) {
  return (
    <div>
      <label className="mb-2 block text-sm font-medium text-slate-500">
      ...
      <div
        onClickCapture={(event) => {
          ...
          event.preventDefault();
          event.stopPropagation();
          onLockedClick(event.currentTarget);
        }}
